...
convert -size 400x150 xc:white -font DejaVu-Sans -pointsize 32 -fill black -draw "text 20,80 'Hello, Project 362 OCR'" test_image.png
...
for i in $(seq 1 $MAX_RETRIES); do
  ...
  # Show resource usage
  ps -p $(cat project-362.pid) -o %cpu,%mem,cmd || true

  # Check for OCR text
  OCR_TEXT=$(sqlite3 $HOME/.project-362/db.sqlite "SELECT text FROM ocr_text;" 2>/dev/null || echo "")

  if echo "$OCR_TEXT" | grep -qi "Hello, Project 362 OCR"; then
    OCR_FOUND=true
    break
  fi
...
if [ "$OCR_FOUND" = true ]; then
  ...
  sqlite3 $HOME/.project-362/db.sqlite "SELECT text FROM ocr_text;" 2>/dev/null | head -20 || echo "(empty)"
  echo ""
