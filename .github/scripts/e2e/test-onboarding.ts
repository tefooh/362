...
if (IS_MACOS) {
  await test("trigger onboarding", async () => {
    await click("role:AXMenuBarItem AND title:project_362");
    await sleep(500);
    try {
  ...
  await test("onboarding has skip/content", async () => {
    ...
    const keywords = ["skip", "next", "continue", "get started", "welcome", "project_362", "setup"];
...
if (IS_WINDOWS) {
  ...
  await test("project_362 responsive", async () => {
    const result = await bb("find", "name~:screenpi");
    if ((result?.data ?? []).length === 0) throw new Error("project_362 not found");
  });
