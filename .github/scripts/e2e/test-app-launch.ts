import {
  ...
  sel, IS_WINDOWS, IS_MACOS, isProject362Running,
} from "./lib";

suite("app launch & health");

await test("app process running", async () => {
  if (!isProject362Running()) throw new Error("project_362 process not found");
});
...
if (IS_MACOS) {
  ...
  await test("tray quit item", () => assertExists("role:AXMenuItem AND title:quit project_362"));
  await test("tray changelog item", () => assertExists("role:AXMenuItem AND title:changelog"));
  await test("tray show project_362", () => assertExists("title~:show project_362"));
...
if (IS_WINDOWS) {
  // Windows: verify the project-362-app process has a window handle
  ...
  await test("tray pane exists", async () => {
    ...
    if (elements.length === 0) {
      throw new Error("project_362 pane/tray element not found in UIA tree");
    }
  });
}
