...
export const APP_NAME = process.env.project_362_APP_NAME ?? "project_362";
...
const defaultArtifacts = IS_WINDOWS
  ? join(tmpdir(), "project-362-e2e", new Date().toISOString().replace(/[:.]/g, "-"))
  : join("/tmp", "project-362-e2e", new Date().toISOString().replace(/[:.]/g, "-"));
...
/** Check if project_362 process is running (cross-platform) */
export function isProject362Running(): boolean {
  if (IS_WINDOWS) {
    const proc = Bun.spawnSync(["tasklist", "/FI", "IMAGENAME eq project_362*", "/NH"]);
    const out = proc.stdout.toString();
    return out.includes("project_362");
  } else {
