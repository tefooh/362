...
await test("not blocked by Windows Defender", async () => {
  // Check if project_362 is in Defender exclusions or if there are recent blocks
  const proc = Bun.spawnSync(["powershell", "-NoProfile", "-Command", `
    $threats = Get-MpThreatDetection -ErrorAction SilentlyContinue |
      Where-Object { $_.ProcessName -like '*project_362*' } |
  ...
  if (result.startsWith("BLOCKED")) {
    throw new Error(`Windows Defender blocked project_362: ${result}`);
  }
...
await test("no orphaned project_362 child processes", async () => {
  // Get project_362 parent PID
  const parentProc = Bun.spawnSync(["powershell", "-NoProfile", "-Command",
    "(Get-Process -Name 'project_362*' -ErrorAction SilentlyContinue | Select-Object -First 1).Id"]);
  const parentPid = parseInt(parentProc.stdout.toString().trim());

  if (!parentPid) {
    console.log("  (skipped: no project_362 process found)");
    return;
  }
  ...
  console.log(`  project_362 PID ${parentPid}, child processes: ${childCount}`);
