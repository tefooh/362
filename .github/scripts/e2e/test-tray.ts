...
import {
  ...
  sel, HEALTH_URL, IS_WINDOWS, IS_MACOS, isProject362Running,
} from "./lib";
...
if (IS_MACOS) {
  ...
  await test("tray icon visible", async () => {
    const result = await bb("find", "role:AXMenuBarItem AND name~:status", "--app", "project-362-app");
  ...
  await test("tray menu opens", async () => {
    ...
    try {
      await bb("click", "role:AXMenuBarItem AND name~:status", "--app", "project-362-app");
      await sleep(1000);
      // Verify menu items appeared
      const items = await bb("find", "role:AXMenuItem", "--app", "project-362-app");
  ...
  await test("tray has all expected items", async () => {
    ...
    const expectedItems = ["show project_362", "settings", "quit project_362"];
  ...
  await test("tray 'show project_362' opens window", async () => {
    // Open tray menu
    await bb("click", "role:AXMenuBarItem AND name~:status", "--app", "project-362-app");
    await sleep(500);

    // Click "show project_362"
    try {
      await bb("click", "role:AXMenuItem AND title~:show project_362", "--app", "project-362-app");
      await sleep(2000);

      // Verify window appeared
      const result = await bb("find", "role:AXWebArea", "--app", "project-362-app");
      const windows = result?.data ?? [];
      if (windows.length === 0) {
        console.log("  warning: window may not have appeared");
      }
    } catch (e: any) {
      console.log(`  (show project_362 interaction failed: ${e.message})`);
    }
  });
  ...
  await test("tray menu does not fire twice (S2.6)", async () => {
    await bb("click", "role:AXMenuBarItem AND name~:status", "--app", "project-362-app");
    await sleep(500);
    const result = await bb("find", "role:AXMenuItem AND title:settings", "--app", "project-362-app");
  ...
  await test("dock icon visible (S2.1)", async () => {
    ...
    // Project362 should appear in visible processes (dock icon = not accessory mode)
    if (!apps.includes("project_362")) {
      console.log("  warning: project_362 not in visible processes (may be in accessory mode)");
    } else {
...
if (IS_WINDOWS) {
  ...
  await test("project_362 process running", async () => {
    if (!isProject362Running()) throw new Error("project_362 not running");
  });

  await test("system tray area accessible", async () => {
    // Windows system tray is tricky — items may be in overflow
    // Try to find project_362 in notification area
    try {
      const result = await bb("find", "name~:screenpi");
      const items = result?.data ?? [];
      if (items.length > 0) {
        console.log(`  found ${items.length} project_362 element(s) in tray area`);
        return;
      }
    } catch {}
  ...
  await test("project_362 window focusable via shortcut", async () => {
  ...
  await test("multiple shortcut toggles don't crash (S2.4)", async () => {
    ...
    if (!isProject362Running()) {
      throw new Error("project_362 crashed after rapid shortcut toggles");
    }
