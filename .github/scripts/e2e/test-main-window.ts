...
if (IS_MACOS) {
  await test("open main window", async () => {
    await bb("activate", "project-362-app");
    await sleep(500);
  ...
  await test("project_362 menu exists", () => assertExists("role:AXMenuBarItem AND title:project_362"));
  await test("edit menu exists", () => assertExists("role:AXMenuBarItem AND title:Edit"));
  await test("about menu item", () => assertExists("role:AXMenuItem AND title:About project_362"));
...
if (IS_WINDOWS) {
  ...
  await test("project_362 process has window", async () => {
    const result = await bb("find", "name~:screenpi");
    const elements = result?.data ?? [];
    if (elements.length === 0) throw new Error("no project_362 elements found");
  });
