 ...
 *
 * These tests kill and restart project_362, so they run LAST.
 */

import {
  suite, test, summary, screenshot, fetchJson, sleep,
  HEALTH_URL, IS_WINDOWS, IS_MACOS, isProject362Running,
} from "./lib";
...
/** Get list of project-362-related PIDs */
function getProject362Pids(): number[] {
  if (IS_WINDOWS) {
    const proc = Bun.spawnSync([
      "powershell", "-NoProfile", "-Command",
      "Get-Process -Name 'project_362*' -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Id",
    ]);
    return proc.stdout.toString().trim().split(/\r?\n/).filter(Boolean).map(Number).filter(n => !isNaN(n));
  } else {
    const proc = Bun.spawnSync(["pgrep", "-f", "project_362"]);
...
/** Find project_362 executable path */
function findProject362Exe(): string | null {
  if (IS_WINDOWS) {
    const proc = Bun.spawnSync(["powershell", "-NoProfile", "-Command",
      "(Get-Process -Name 'project_362*' -ErrorAction SilentlyContinue | Select-Object -First 1).Path"]);
    const path = proc.stdout.toString().trim();
    return path || null;
  } else {
    const proc = Bun.spawnSync(["bash", "-c", "ps aux | grep -i project_362 | grep -v grep | head -1 | awk '{print $11}'"]);
    const path = proc.stdout.toString().trim();
    return path || null;
  }
}

/** Start project_362 app (best effort) */
function startProject362(exePath: string) {
  if (IS_WINDOWS) {
...
const exePath = findProject362Exe();
console.log(`  project_362 exe: ${exePath ?? "not found"}`);
...
await test("CPU not spiking on idle (10s sample)", async () => {
  if (IS_WINDOWS) {
    // Measure project_362 CPU over 10s
    const proc = Bun.spawnSync(["powershell", "-NoProfile", "-Command", `
      $samples = @()
      for ($i = 0; $i -lt 5; $i++) {
        $cpu = (Get-Process -Name 'project_362*' -ErrorAction SilentlyContinue | Measure-Object -Property CPU -Sum).Sum
        $samples += $cpu
    ...
    const proc = Bun.spawnSync(["bash", "-c",
      "ps aux | grep -i project_362 | grep -v grep | awk '{sum+=$3} END {print sum}'"]);
...
await test("force quit + recovery (DB intact)", async () => {
  if (!exePath) {
    console.log("  (skipped: could not find project_362 exe path)");
    return;
  }
  ...
  // Force kill project_362
  console.log("  force killing project-362...");
  if (IS_WINDOWS) {
    Bun.spawnSync(["powershell", "-NoProfile", "-Command",
      "Get-Process -Name 'project_362*' -ErrorAction SilentlyContinue | Stop-Process -Force"]);
  } else {
    Bun.spawnSync(["pkill", "-9", "-f", "project-362-app"]);
  }
  await sleep(3000);

  // Verify it's dead
  if (isProject362Running()) {
    throw new Error("project_362 still running after force kill");
  }

  // Restart
  console.log("  restarting project-362...");
  startProject362(exePath);
...
await test("disk space check (S9.6)", async () => {
  ...
  if (!isProject362Running()) throw new Error("app not running");
});
...
await test("clean quit leaves no orphans (S8.1)", async () => {
  ...
  const pidsBefore = getProject362Pids();
  if (pidsBefore.length === 0) {
    console.log("  (skipped: project_362 not running)");
    return;
  }
  ...
  if (IS_WINDOWS) {
    ...
    Bun.spawnSync(["pkill", "-TERM", "-f", "project-362-app"]);
  }
  await sleep(10_000);

  // Check all processes exited
  const pidsAfter = getProject362Pids();
  ...
  const recovered = await waitForHealthUp(60);
  if (!recovered) {
    throw new Error("could not restart project_362 after clean quit test");
  }
});
...
await test("check logs for slow DB warnings (S9.1)", async () => {
  // Look for project_362 logs and check for slow insert warnings
  let logContent = "";
  if (IS_WINDOWS) {
    const proc = Bun.spawnSync(["powershell", "-NoProfile", "-Command", `
      $logDir = Join-Path $env:LOCALAPPDATA 'project_362'
    ...
    const proc = Bun.spawnSync(["bash", "-c",
      "tail -500 ~/Library/Logs/project-362/*.log 2>/dev/null || echo NO_LOG"]);
    logContent = proc.stdout.toString();
  }
