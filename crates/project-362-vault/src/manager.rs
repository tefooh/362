...
impl VaultManager {
    pub fn new(project_362_dir: PathBuf) -> Self {
        let initial_state = if !project_362_dir.join("vault.meta").exists() {
            VaultState::None
        } else if project_362_dir.join(".vault_locked").exists() {
            // Sentinel file present → vault is locked
            VaultState::Locked
        } else {
            // Vault set up but no lock sentinel — check if DB is actually encrypted
            let db_path = project_362_dir.join("db.sqlite");
    ...
    pub fn is_set_up(&self) -> bool {
        self.project_362_dir.join("vault.meta").exists()
    }
    ...
    pub async fn setup(&self, password: &str) -> VaultResult<()> {
        ...
        let meta_path = self.project_362_dir.join("vault.meta");
    ...
    pub async fn lock(&self, password: &str) -> VaultResult<watch::Receiver<MigrationProgress>> {
        ...
        let project_362_dir = self.project_362_dir.clone();
        let data_dir = self.project_362_dir.join("data");
        let db_path = self.project_362_dir.join("db.sqlite");
        let sentinel = self.project_362_dir.join(".vault_locked");
        ...
        *self.state.write().await = VaultState::Migrating { progress: 0.0 };
        ...
        tokio::spawn(async move {
            ...
            if data_dir.exists() {
                if let Err(e) =
                    migration::encrypt_data_dir(&project_362_dir, &data_dir, key_bytes, progress_tx)
                        .await
                {
    ...
    pub async fn unlock(&self, password: &str) -> VaultResult<watch::Receiver<MigrationProgress>> {
        ...
        *self.state.write().await = VaultState::Migrating { progress: 0.0 };
        ...
        tokio::spawn(async move {
            ...
            if data_dir.exists() {
                if let Err(e) =
                    migration::decrypt_data_dir(&project_362_dir, &data_dir, key_bytes, progress_tx)
                        .await
                {
