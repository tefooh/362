...
use project_362_core::connections::sync as core_connections_sync;
...
pub trait Integration: Send + Sync {
    ...
    ///
    /// The proxy handler in project-362-engine and the integration's own
...
// ---------------------------------------------------------------------------
// Credential store (~/.project-362/connections.json)
// ---------------------------------------------------------------------------
...
pub fn load_store(project_362_dir: &Path) -> HashMap<String, SavedConnection> {
...
pub fn save_store(project_362_dir: &Path, data: &HashMap<String, SavedConnection>) -> Result<()> {
    let path = store_path(project_362_dir);
...
pub async fn load_connection(
    ...
    let file_store = load_store(project_362_dir);
...
pub async fn is_connection_configured(
    ...
    if integration.oauth_config().is_some() {
        ...
        load_connection(secret_store, project_362_dir, conn_id)
            .await
...
async fn save_connection(
    ...
    let mut file_store = load_store(project_362_dir);
    file_store.insert(key.to_string(), conn.clone());
    save_store(project_362_dir, &file_store)
}
...
impl ConnectionManager {
    ...
    pub async fn connect(&self, id: &str, creds: Map<String, Value>) -> Result<()> {
        ...
        if result.is_ok() {
            core_connections_sync::clear_connection_tombstone(&self.project_362_dir, id);
        }
    ...
    pub async fn get_credentials_instance(
        ...
        Ok(
            load_connection(self.secret_store.as_deref(), &self.project_362_dir, &key)
                .await
                .map(|c| c.credentials),
        )
    }
    ...
    pub async fn disconnect(&self, id: &str) -> Result<()> {
        let result =
            remove_connection(self.secret_store.as_deref(), &self.project_362_dir, id).await;
        if result.is_ok() {
            core_connections_sync::record_connection_tombstone(&self.project_362_dir, id);
        }
        result
    }
    ...
    pub async fn connect_instance(
        ...
        if result.is_ok() {
            core_connections_sync::clear_connection_tombstone(&self.project_362_dir, &key);
        }
    ...
    pub async fn get_all_instances(
        ...
        self.find(id)?;
        Ok(load_all_instances(self.secret_store.as_deref(), &self.project_362_dir, id).await)
    }
    ...
    pub async fn disconnect_instance(&self, id: &str, instance: Option<&str>) -> Result<()> {
        let key = make_key(id, instance);
        let result =
            remove_connection(self.secret_store.as_deref(), &self.project_362_dir, &key).await;
        if result.is_ok() {
            core_connections_sync::record_connection_tombstone(&self.project_362_dir, &key);
        }
        result
    }
...
pub async fn render_context(
    ...
    for integration in integrations.iter().filter(|i| i.oauth_config().is_none()) {
        ...
        for (instance, conn) in load_all_instances(secret_store, project_362_dir, def.id).await {
    ...
    let ics_settings =
        crate::ics_calendar::load_ics_calendar_settings_from_store(project_362_dir).ok();
...
async fn load_all_instances(
    ...
    let store = load_store(project_362_dir);
...
mod tests {
    ...
    fn temp_project_362_dir() -> PathBuf {
        let dir = std::env::temp_dir().join(format!(
            "project-362-connect-test-{}-{}",
            std::process::id(),
