...
impl WhatsAppGateway {
    ...
    async fn ensure_deps(&self, bun_path: &str) -> Result<()> {
        let session_dir = self.project_362_dir.join("whatsapp-session");
        std::fs::create_dir_all(&session_dir)?;

        let script_path = self.project_362_dir.join("whatsapp-gateway.mjs");
        ...
        if !node_modules.exists() {
            ...
            install_cmd
                ...
                .current_dir(&self.project_362_dir)
                .stdout(Stdio::null())
                .stderr(Stdio::piped());
            #[cfg(windows)]
    ...
    fn spawn_watchdog(&self, my_generation: u64) {
        ...
        tokio::spawn(async move {
            loop {
                ...
                let has_session = project_362_dir
                ...
                *status.lock().await = WhatsAppStatus::Error {
                ...
                // Re-spawn
                let session_dir = project_362_dir.join("whatsapp-session");
                let script_path = project_362_dir.join("whatsapp-gateway.mjs");
...
///      `project_362_core::agents::pi::find_bun_executable` — this is the
...
pub fn resolve_bun_path(hint: Option<&str>) -> Result<String> {
    ...
    if let Some(p) = project_362_core::agents::pi::find_bun_executable() {
        return Ok(p);
    }
    ...
    anyhow::bail!(
        "bun binary not found. project_362 normally ships bun bundled inside the \
