...
//! Pure-Rust SSH/SFTP sync for project_362 data.
//!
//! Copies the entire ~/.project-362 directory to a remote server over SFTP.
...
/// everything else is literal. Enough for `.project-362ignore` entries like
...
/// Project362 skill file dropped on the remote so AI agents know how to query the data.
const project_362_SKILL: &str = r#"# Project362 Data Reference

This directory contains synced data from [project_362](https://project362.com) — an AI that knows everything you've seen, said, or heard.
...
/// Used to wire an agent (e.g. `project_362 agent setup <target>`) on a VPS
...
async fn sync_dir(
    ...
    while let Ok(Some(entry)) = entries.next_entry().await {
        ...
        // User selectivity (--no-media / --exclude / .project-362ignore), on top
...
/// Sync the entire ~/.project-362 directory to the remote server.
...
/// (`--no-media`, `--exclude`, `.project-362ignore`). The plain
...
async fn sync_to_remote_inner(
    ...
    let skill_path = format!("{}/project-362.md", remote_base);
    if let Ok(mut f) = sftp.create(&skill_path).await {
        let _ = f.write_all(project_362_SKILL.as_bytes()).await;
...
pub async fn discover_ssh_hosts() -> Vec<DiscoveredHost> {
    ...
    // mDNS discovery — find project_362 instances advertising on LAN
