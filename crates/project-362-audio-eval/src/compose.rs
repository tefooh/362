...
//!
//! VoxConverse clips are short and clean. Real project_362 sessions are long
    ...
    for (idx, segment) in template.segments.iter().enumerate() {
        ...
        match segment {
            ...
            Segment::Audio {
                ...
                let (raw, source_rate) = project_362_audio::pcm_decode(&source_path)
                    .with_context(|| format!("decode audio: {}", source_path.display()))?;
                let resampled = if source_rate != COMPOSE_SAMPLE_RATE {
                    project_362_audio::resample(&raw, source_rate, COMPOSE_SAMPLE_RATE)
                        .with_context(|| {
                            format!(
                                "resample {} {}->{}",
                                source_path.display(),
