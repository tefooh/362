...
//!
//! Runs the project_362 diarization chain (VAD → segmentation → embedding →
...
//!
//! Drives `prepare_segments` + `EmbeddingManager` directly via `project-362-audio`'s
...
use project_362_audio::speaker::embedding::EmbeddingExtractor;
use project_362_audio::speaker::embedding_manager::EmbeddingManager;
use project_362_audio::speaker::prepare_segments;
use project_362_audio::vad::{silero::SileroVad, VadEngine};
use project_362_audio_eval::{load_rttm, score_pipeline, RttmSegment};
...
#[derive(Parser, Debug)]
#[command(about = "Score project_362 diarization against an RTTM reference")]
struct Args {
...
async fn main() -> Result<()> {
    ...
    let (samples, source_rate) = project_362_audio::pcm_decode(&args.audio)?;
    ...
    // Models live in project-362-audio's models dir (kept there so the prod
    // app and eval crate can share the same on-disk artifacts).
    let project_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let audio_crate_dir = project_dir.parent().unwrap().join("project-362-audio");
    ...
    if !segmentation_model_path.exists() || !embedding_model_path.exists() {
        anyhow::bail!(
            "missing pyannote models at {} / {}. Run project_362 once to download them, \
             or copy them into crates/project-362-audio/models/pyannote/.",
