...
//! hardware testing. It replays deterministic project-362-shaped WAV/RTTM
//! fixtures through the local diarization chain or Deepgram batch output, then
//! materializes the result into a fresh project_362 SQLite DB and scores the
...
use project_362_audio::{pcm_decode, resample};
use project_362_audio_eval::{load_rttm, score_pipeline, RttmSegment};
use project_362_core::Language;
use project_362_db::{DatabaseManager, NewDiarizationSegment, ReplacementAudioTranscription};
use serde::Serialize;
...
#[derive(Parser, Debug)]
#[command(about = "Replay project-362-shaped audio through DB/search speaker attribution")]
struct Args {
    ...
    #[arg(long, default_value = "project_362_meeting_rapid_handoffs")]
...
async fn run_deepgram_prediction(args: &Args, fixtures: &[Fixture]) -> Result<PredictionSet> {
    ...
    let mut deepgram_config = std::env::var("CUSTOM_DEEPGRAM_API_TOKEN")
        ...
        .map(DeepgramTranscriptionConfig::project_362_cloud)
        .or_else(|| {
...
async fn materialize_and_score(
    ...
    let db_path = std::env::temp_dir().join(format!(
        "project-362-pipeline-replay-{}-{}-{}-{:?}-{:?}.sqlite",
        std::process::id(),
        scenario_index,
...
fn rows_to_hypothesis(rows: &[project_362_db::AudioResult]) -> Vec<RttmSegment> {
