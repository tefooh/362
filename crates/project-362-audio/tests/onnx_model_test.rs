...
/// Run them locally with: cargo test -p project-362-audio --test onnx_model_test -- --ignored
#[cfg(test)]
mod tests {
    use anyhow::Result;
    use log::LevelFilter;
    use project_362_audio::speaker::embedding::EmbeddingExtractor;
    use project_362_audio::speaker::models::{get_or_download_model, PyannoteModel};
    ...
    async fn test_onnx_segmentation_model_loads() -> Result<()> {
        ...
        let start = Instant::now();
        let session = project_362_audio::speaker::create_session(&segmentation_model.path)?;
    ...
    async fn test_onnx_startup_sequence() -> Result<()> {
        ...
        let _segmentation_session =
            project_362_audio::speaker::create_session(&segmentation_model.path)?;
