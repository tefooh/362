...
use project_362_db::{DatabaseManager, NewDiarizationSegment, Speaker};
...
pub async fn process_transcription_result(
    ...
    // to recover instead of silently dropping the audio (project-362-CLI-RC).
            ...
            .insert_audio_chunk_and_transcription(
                ...
                &project_362_db::AudioDevice {
                    ...
                    device_type: match result.input.device.device_type {
                        crate::core::device::DeviceType::Input => project_362_db::DeviceType::Input,
                        crate::core::device::DeviceType::Output => {
                            project_362_db::DeviceType::Output
                        }
                    },
                },
                speaker_id,
            ...
            .await
        {
            ...
            Err(e) => {
                if retry < 2 {
                    ...
                    // Whisper pass). project-362-CLI-RC. device is a structured
...
mod tests {
    ...
    // project-362-CLI-RC: when the chunk+transcription insert exhausts its
