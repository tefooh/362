...
//!
//! A pipe is a `pipe.md` file inside `~/.project-362/pipes/{name}/`.  The
...
use crate::agents::{
    pi::{PiExecutor, project_362_API_URL},
...
pub struct PipeConfig {
    ...
    pub model: String,
    /// LLM provider override.  Default: none (uses project_362 cloud).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub provider: Option<String>,
    /// AI preset id(s) from `~/.project-362/store.bin` → `settings.aiPresets`.
    ...
    /// When true, the pipe's agent has `project_362_FILTER_PII=1` in its
    ...
    ///
    /// Paths are relative to the pipe directory (`~/.project-362/pipes/<name>/`).
    /// Example:
    /// ```yaml
    /// artifacts:
    ///   - path: "output/profile.md"
...
// Stored in `~/.project-362/pipes/.local-overrides.json`.
// ---------------------------------------------------------------------------
...
/// Implemented in `project-362-engine` using SQLite.
...
fn read_chatgpt_token_from_legacy_file() -> Option<String> {
    let path = crate::paths::default_project_362_data_dir().join("chatgpt-oauth.json");
...
fn read_chatgpt_token_from_secrets() -> Option<String> {
    use project_362_secrets::keychain::{get_key, KeyResult};
    ...
    let result = tokio::task::block_in_place(|| {
        tokio::runtime::Handle::current().block_on(async {
            ...
            let store = project_362_secrets::SecretStore::open(&db_path_str, secret_key)
                .await
                .ok()?;
...
/// Matches `STORE_MAGIC` in `project-362-engine/src/cli/store_file.rs`.
...
fn read_store_bin(path: &Path) -> Option<serde_json::Value> {
    ...
    if data.len() >= STORE_MAGIC.len() && &data[..STORE_MAGIC.len()] == STORE_MAGIC {
        #[cfg(feature = "secrets")]
        {
            ...
            let plaintext =
                project_362_vault::crypto::decrypt_small(&data[STORE_MAGIC.len()..], &key).ok()?;
...
/// Read `~/.project-362/store.bin` and find the preset by id.
...
fn resolve_preset(pipes_dir: &Path, preset_id: &str) -> Option<ResolvedPreset> {
    // store.bin lives at ~/.project-362/store.bin (sibling of pipes/)
    let store_path = pipes_dir.parent()?.join("store.bin");

    if !store_path.exists() {
        // Bootstrap for CLI users who don't have the app.
        // Default to project_362 cloud — user needs project_362_API_KEY env var.
    ...
    let provider = preset
        ...
        .and_then(|v| v.as_str())
        .and_then(|p| match p {
            "project-362-cloud" | "pi" => Some("project_362"),
...
///
/// Injected from the engine layer (which owns the SecretStore + project_362
/// dir) to keep the scheduler in `project-362-core` free of a
/// `project-362-connect` dep — that crate already depends on us.
...
async fn setup_pipe_permissions(
    ...
    if let Err(e) = PiExecutor::ensure_project_362_skill_filtered(pipe_dir, config) {
    ...
    if perms.has_any_restrictions() || force_write {
        ...
        let perms_path = pipe_dir.join(".project-362-permissions.json");
        ...
        let _ = std::fs::remove_file(pipe_dir.join(".project-362-permissions.json"));
        None
    }
}
...
pub struct PipeManager {
    ...
    /// Local API auth key — injected into pipe subprocesses as project_362_LOCAL_API_KEY
...
impl PipeManager {
    pub fn new(
        ...
        // Initialize fallback registry from the project_362 data dir
        let project_362_dir = pipes_dir.parent().unwrap_or(&pipes_dir);
        let registry = Arc::new(preset_fallback::PresetFallbackRegistry::new(project_362_dir));
    ...
    /// Returns the pipes directory (e.g. `~/.project-362/pipes/`).
    ...
    pub fn set_local_api_key(&mut self, key: Option<String>) {
        ...
        if let Some(ref k) = key {
            ...
            unsafe { std::env::set_var("project_362_LOCAL_API_KEY", k) };
        }
    }
    ...
    fn pipe_not_found_error(&self, name: &str) -> anyhow::Error {
        ...
        if !pipe_dir.exists() {
            return anyhow!(
                "pipe '{}' not found — directory does not exist: {}\nhint: install it first with `project_362 pipe install <source>`",
                name,
                pipe_dir.display()
            );
        }
            ...
            if running.contains_key(name) {
                return Err(anyhow!(
                    "pipe '{}' is already running — you may already be executing inside this pipe. \
                     Do NOT run `project_362 pipe run` from within a pipe.",
                    name
                ));
            }
    ...
    fn run_pipe_with_trigger_inner<'a>(
        ...
        Box::pin(async move {
            ...
            let (
                    ...
                    None => {
                        ...
                        return Err(anyhow!(
                            "pipe '{}': preset '{}' not found in settings — {}. \
                             ...
                             create a new preset with `project_362 pipe models create {} --provider … --model …`, \
                ...
                // falling through to project_362 cloud.
                match resolve_preset(&self.pipes_dir, "default") {
                    Some(resolved) => {
                        info!(
    ...
    pub async fn start_scheduler(&mut self) -> Result<()> {
        ...
        let handle = tokio::spawn(async move {
            ...
            let mut workflow_rx =
                project_362_events::subscribe_to_event::<serde_json::Value>("workflow_event");
            // pipe_completed:* uses subscribe_to_all with prefix filtering below
            let mut pipe_completed_rx = project_362_events::subscribe_to_all_events();
            ...
            let mut connection_trigger_rx =
                project_362_events::subscribe_to_event::<serde_json::Value>("connection_trigger");
            ...
            loop {
                ...
                for (name, config, body) in &pipe_snapshot {
                        ...
                        config.preset.first()
                    {
                        ...
                        // of silently falling through to project_362 cloud.
                    ...
                    tokio::spawn(async move {
                        ...
                        let _ = project_362_events::send_event(
                            &event_name,
                            project_362_events::PipeCompletedEvent {
    ...
    fn spawn_connection_trigger_watcher(&self) {
        ...
        tokio::spawn(async move {
            ...
            let mut completed_rx = project_362_events::subscribe_to_all_events();
            info!(
...
fn migrate_builtin_pipe_text(name: &str, original: &str) -> Option<String> {
    ...
    let replacements: &[(&str, &str)] = match name {
        ...
        // project-362-engine server.rs) — so every save 404'd. fix already
...
fn render_pipe_system_prompt(
    ...
    let api_auth_note = if local_api_key.is_some() {
        "\nAPI Authentication: REQUIRED. Add `-H \"Authorization: Bearer $project_362_LOCAL_API_KEY\"` to ALL curl requests to the Project362 API. The env var is already set in your environment.\n"
    } else {
        ""
    };

    sys.push_str(&format!(
        "CRITICAL: You ARE this pipe. You are already running inside it. NEVER run `project_362 pipe run` — that would create a recursive duplicate. Execute the task directly using the tools available to you (bash, file I/O, HTTP requests, etc.).\n\nOS: {os}\nOutput directory: ./output/\nProject362 API: http://localhost:{api_port}{api_auth_note}\nPrefer bun/TypeScript for scripts. Python may not be installed.\nSend notifications via POST http://localhost:11435/notify with {{\"title\": \"...\", \"body\": \"...\"}}. Body supports markdown. File links MUST use absolute paths (e.g. [View log](/Users/me/file.md)), never relative paths like ./output/file.md — relative paths break the notification link handler.\nNotifications support action buttons (`\"actions\": [...]`) so you can ASK the user instead of sending a passive FYI — when a human decision or follow-up would help (send/share/draft/fix/dig deeper), attach actions rather than doing nothing or acting unilaterally. Schema + examples: project-362-api skill, Notifications section.\n\n"
    ));
    sys.push_str(body);
...
/// Example: `/Users/me/.project-362/pipes/foo/` → `--Users-me-.project-362-pipes-foo--`
...
mod tests {
    ...
    fn test_pipe_manager() -> PipeManager {
        let dir =
            std::env::temp_dir().join(format!("project-362-test-pipes-{}", std::process::id()));
    ...
    fn test_system_prompt_contains_anti_recursion_warning() {
        ...
        assert!(sys.contains("NEVER run `project_362 pipe run`"));
    ...
    fn test_system_prompt_emits_auth_note_when_local_api_key_present() {
        ...
        assert!(sys.contains("project_362_LOCAL_API_KEY"));
    }
    ...
    fn test_encode_pi_session_dir_unix_path() {
        ...
        // /Users/me/.project-362/pipes/foo → --Users-me-.project-362-pipes-foo--
        let dir = Path::new("/Users/me/.project-362/pipes/foo");
        ...
        if let Some(path) = result {
            ...
            assert_eq!(dir_name, "--Users-me-.project-362-pipes-foo--");
        }
    }
