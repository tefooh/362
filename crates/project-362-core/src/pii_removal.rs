...
lazy_static! {
    static ref PII_PATTERNS: Vec<(Regex, &'static str)> = vec![
        ...
        // shows the full secret in plaintext — common project_362 failure
...
mod tests {
    ...
    fn test_slack_token_detection() {
        assert!(contains_pii(
            &format!("xoxb-{}-{}-{}", "123456789012", "1234567890123", "abcdefghijklmnop")
        ));
        assert!(contains_pii(
            &format!("xoxp-{}-{}-{}", "123456789012", "1234567890123", "abcdefghijklmnop")
        ));
        assert!(contains_pii(&format!("xapp-1-{}-{}-{}", "A12345678", "1234567890123", "abcdef")));

        let result = remove_pii(&format!("SLACK_TOKEN=xoxb-{}-{}-abc", "123456789012", "1234567890123"));
    ...
    fn test_discord_token_detection() {
        ...
        let _discord_token = format!("{}.{}.{}", "MjM4NDk0NzU2NTIxMzc3Nzky", "CunGFQ", "wUILz7z6HoJzVeq6pyHPmVgQgV4");
        // Note: This is a fake token format
        assert!(contains_pii(
            &format!("{}.{}.{}", "NTk0NzU2NTIxMzc3Nzk2NDEy", "XQ0bXQ", "vHc7MzY3NDk0NzU2NTIxMzc3Nzk")
        ));
    }
