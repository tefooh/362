...
try {
  ...
  const filePath = path.join(process.cwd(), ".project-362-permissions.json");
...
function hitsProject362Api(cmd: string): boolean {
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("tool_call", async (event: any) => {
    ...
    if (hitsProject362Api(cmd) && !isParsableCurl(cmd)) {
      return {
        block: true,
        reason: "Use curl to access the Project362 API. Other HTTP clients are not supported with permissions enabled.",
      };
    }
    if (isParsableCurl(cmd) && hitsProject362Api(cmd)) {
