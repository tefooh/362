...
pub struct TinfoilImageConfig {
    ...
    /// `project_362_PRIVACY_FILTER_ENCLAVE`, then `project_362_PRIVACY_FILTER_URL`,
    ...
    /// to `project_362_PRIVACY_FILTER_REPO`, then [`DEFAULT_REPO`].
    pub repo: Option<String>,
    /// Bearer token. Falls back through `project_362_PRIVACY_FILTER_API_KEY`,
    /// `project_362_LOCAL_API_KEY`, then `project_362_API_AUTH_KEY` (deprecated alias).
            ...
            None => {
                tracing::info!(
                    "tinfoil_image redactor has no api key — requests will be un-authenticated. \
                     Set project_362_PRIVACY_FILTER_API_KEY or pass via TinfoilImageConfig.api_key."
                );
                (None, false)
            }
        };
