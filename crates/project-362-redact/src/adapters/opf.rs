...
//! `project-362-pii-redactor-runtime/results/onnx_export_2026-05-08.md`):
...
//! MPS on Metal (41 ms vs 40 ms p50). See `project-362/opf-rs` for the
//! full benchmark.
//!
//! ## Status
//!
...
//! `apps/project-362-app-tauri/src-tauri/src/server_core.rs`); this
...
//! translate to project-362-redact's `RedactionOutput` (same shape, but
...
/// canonical project_362 install layout.
#[derive(Debug, Clone)]
pub struct OpfConfig {
    /// Directory containing `config.json` + `model.safetensors`.
    /// Default: `~/.project-362/models/opf-v6`. First-run is created
