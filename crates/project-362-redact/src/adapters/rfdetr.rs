...
//! [project-362-pii-bench-image](https://github.com/project-362/project-362-pii-bench-image)
...
//! quoting.
//!
//! Bench accuracy on `project-362-pii-bench-image` val (221 images):
...
//! recipe: `project-362-pii-bench-image/training/export_fp16.py`.
...
/// `project-362-pii-bench-image/src/score.py::CANONICAL_LABELS`.
#[cfg(feature = "onnx-cpu")]
...
pub struct RfdetrConfig {
    /// Path to `rfdetr_vN.onnx`. We default to `~/.project-362/models/rfdetr_v13.onnx`
...
impl RfdetrConfig {
    /// `~/.project-362/models/rfdetr_v13.onnx`. Created lazily by
...
mod tests {
    ...
    #[test]
    fn default_path_lives_under_project_362_dir() {
        let p = RfdetrConfig::default_model_path();
        let expected_suffix = Path::new(".project-362")
            .join("models")
            .join("rfdetr_v13.onnx");
        assert!(
