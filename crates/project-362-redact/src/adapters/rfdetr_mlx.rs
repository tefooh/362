...
//! RF-DETR-Nano image-PII detector via [`project_362_rfdetr_mlx`] —
...
//! `huggingface.co/project-362/pii-image-redactor` on first run
//! (~108 MB), verifies SHA-256, atomic-renames into
//! `~/.project-362/models/rfdetr_v9.safetensors`. Subsequent starts
//! are instant.
...
///
/// Mirrors `project_362_config::PARAKEET_MIN_MACOS_MAJOR` — mlx-sys's
...
/// `project_362_config::macos_major_version`, inlined here so this
/// crate doesn't pick up project-362-config just for one fn. On any
...
pub struct RfdetrMlxConfig {
    ...
    /// `~/.project-362/models/rfdetr_v9.safetensors`.
...
mod imp {
    ...
    /// pipeline (`project-362-pii-bench-image/src/score.py`).
    ...
    type ModelHandle = Arc<StdMutex<project_362_rfdetr_mlx::RfDetr>>;
    ...
    impl RfdetrMlxRedactor {
        ...
        async fn ensure_loaded(&self) -> Result<ModelHandle, RedactError> {
            ...
            if state.model.is_none() {
                ...
                let model = tokio::task::block_in_place(|| {
                    let mut m = project_362_rfdetr_mlx::RfDetr::load(&path)
        ...
        async fn infer(&self, image_path: &Path) -> Result<Vec<ImageRegion>, RedactError> {
            ...
            tokio::task::block_in_place(move || {
                ...
                // project-362-rfdetr-mlx already excludes it.
...
mod tests {
    ...
    #[test]
    fn default_path_under_project_362_dir() {
        let p = RfdetrMlxConfig::default_model_path();
        let s = p.to_string_lossy();
        assert!(s.contains(".project-362/models/rfdetr_v9.safetensors"));
    }
