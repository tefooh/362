...
machine (`project_362_engine::meeting_detector::advance_state`) and
...
```bash
# all seed scenarios
cargo run --release -p project-362-meeting-eval --bin project-362-eval-meeting-state -- \
  crates/project-362-meeting-eval/evals/scenarios/*.toml

# unit tests (includes embedded scenarios)
cargo test -p project-362-meeting-eval
```
...
`project-362-eval-meeting-replay-trace` consumes a JSONL trace — one
`{t, in_call, has_audio?}` per line — and feeds it through the same
state machine the TOML scenarios use:

```bash
cargo run --release -p project-362-meeting-eval --bin \
  project-362-eval-meeting-replay-trace -- \
  crates/project-362-meeting-eval/evals/traces/meeting72_arc_real.jsonl \
...
`~/.project-362/project-362-app.2026-05-11.log` — the user's 70-minute
...
appends one JSON line per scan to `~/.project-362/meeting_traces/`.
