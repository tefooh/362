...
use project_362_engine::video_cache::FrameCache;
...
async fn setup_test_env() -> Result<(FrameCache, Arc<DatabaseManager>)> {
    ...
    debug!("using real project_362 data dir: {:?}", project_362_dir);
    ...
    let cache = FrameCache::new(project_362_dir, db.clone()).await?;
    Ok((cache, db))
}
