...
mod tests {
    ...
    /// TEST 9: Test project_362 app filtering logic
    ///
    /// Frames from project_362 app should be filtered out.
    /// This tests the filtering function directly.
    #[tokio::test]
    async fn test_project_362_app_filtering() {
        // Simulate the filtering logic from create_time_series_frame
        fn should_include_entry(app_name: &str) -> bool {
            !app_name.to_lowercase().contains("project_362")
        }
        ...
        // Project362 variants should be filtered out
        assert!(
            !should_include_entry("project_362"),
            "project_362 should be filtered"
        );
        assert!(
            !should_include_entry("Project362"),
            "Project362 should be filtered"
        );
        assert!(
            !should_include_entry("project_362"),
            "project_362 should be filtered"
        ...
        assert!(
            !should_include_entry("project-362-app"),
            "project-362-app should be filtered"
        );
    }

    /// TEST 10: Test that frames with all-project-362 entries result in empty frame_data
    ///
    /// When all OCR entries in a frame are from project_362, after filtering,
    /// the frame_data should be empty and the frame should NOT be sent.
    #[tokio::test]
    async fn test_all_project_362_entries_results_in_empty_frame() {
        // Simulate entries from a frame
        let entries = vec![
            ("project_362", "main window"),
            ("project_362", "search"),
            ("project_362", ""),
        ];
        ...
        let filtered: Vec<_> = entries
            .into_iter()
            .filter(|(app_name, _)| !app_name.to_lowercase().contains("project_362"))
            .collect();

        assert!(
            filtered.is_empty(),
            "All project_362 entries should be filtered out, resulting in empty frame_data"
        );
    ...
    /// TEST 11: Test mixed entries (some project_362, some other apps)
    ///
    /// When a frame has mixed entries, only non-project-362 entries should remain.
    #[tokio::test]
    async fn test_mixed_entries_partial_filtering() {
        ...
        assert_eq!(filtered.len(), 2, "Should have 2 non-project-362 entries");
    ...
    async fn test_empty_frame_data_should_be_skipped() {
        ...
        // Frame with empty data (all project_362 filtered) - should NOT be sent
