...
use project_362_engine::video_utils::extract_frame_from_video;

/// Setup test environment with real project_362 database
async fn setup_test_env() -> Result<Arc<DatabaseManager>> {
    ...
    let db = Arc::new(
        DatabaseManager::new(
            paths::default_project_362_data_dir()
                .join("db.sqlite")
                .to_str()
                .unwrap(),
            Default::default(),
...
#[tokio::test]
#[ignore = "requires real project_362 data, run with --ignored"]
...
#[tokio::test]
#[ignore = "requires running project_362 server, run with --ignored"]
