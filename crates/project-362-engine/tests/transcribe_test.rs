...
mod tests {
    ...
    async fn setup_test_app() -> Router {
        ...
        let audio_manager = Arc::new(
            AudioManagerBuilder::new()
                .output_path("/tmp/project-362-test".into())
                .build(db.clone())
                .await
                .unwrap(),
        );
        ...
        let app = SCServer::new(
            ...
            PathBuf::from("/tmp/project-362-test"),
            false,
            false,
            audio_manager,
            false,
    ...
    async fn test_transcribe_real_audio_returns_text() {
        ...
        // Use the test WAV file from project-362-audio
        let test_file = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .unwrap()
            .join("project-362-audio/test_data/poetic_kapil_gupta.wav");
