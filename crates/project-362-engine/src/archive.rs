...
use project_362_core::sync::{BlobType, DownloadedBlob, SyncClientConfig, SyncManager};
...
pub struct ArchiveDownloadRequest {
    ...
    pub token: String,
    /// Destination folder. Defaults to `<project_362_dir>/archive-export`.
...
fn archive_password(token: &str) -> String {
    format!("project-362-archive-{:x}", md5::compute(token.as_bytes()))
}
...
pub async fn archive_download(
        ...
        .filter(|s| !s.is_empty())
    {
        ...
        None => state.project_362_dir.join("archive-export"),
    };
...
/// `project-362-core`'s sync service.
#[derive(Debug)]
struct QuotaBackoff {
    strikes: u32,
...
mod tests {
    ...
    fn archive_password_is_stable_and_token_specific() {
        ...
        assert!(archive_password("tok-123").starts_with("project-362-archive-"));
