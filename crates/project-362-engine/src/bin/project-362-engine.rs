...
async fn doctor_check_system_audio_capture() -> bool {
    ...
    match get_cpal_device_and_config(&device).await {
        ...
        Err(e) => {
            let terminal = project_362_core::permissions::detect_terminal();
            eprintln!("  system audio capture: failed");
            eprintln!("    ScreenCaptureKit could not open System Audio for this CLI session.");
            eprintln!(
                "    macOS grants CLI capture through the terminal app, not the project_362 app."
            );
            eprintln!(
            ...
            eprintln!(
                "    if you only need a workaround: project_362 record --experimental-coreaudio-system-audio"
            );
...
fn get_base_dir(custom_path: &Option<String>) -> anyhow::Result<PathBuf> {
    let default_path = paths::default_project_362_data_dir();
...
fn setup_logging(
    ...
    let file_appender = project_362_engine::logging::SizedRollingWriter::builder()
    ...
    let make_env_filter = || {
        ...
        let filter = env::var("project_362_LOG")
            .unwrap_or_default()
            .split(',')
            .filter(|s| !s.is_empty())
        ...
        if debug {
            filter.add_directive("project_362=debug".parse().unwrap())
        } else {
            filter
        }
    };
...
async fn main() -> anyhow::Result<()> {
    ...
    project_362_engine::fd_limit::set_fd_limit();

    debug!("starting project_362 server");
    ...
    let record_args = match cli.command {
        ...
        Command::Agent { ref subcommand } => {
            project_362_engine::cli::agent::handle_agent_command(subcommand).await?;
            return Ok(());
        }
        Command::Service { ref subcommand } => {
            project_362_engine::cli::service::handle_service_command(subcommand).await?;
            return Ok(());
        }
        ...
        Command::Connection { ref subcommand } => {
            project_362_engine::cli::connection::handle_connection_command(subcommand).await?;
            return Ok(());
        }
        Command::Vault { ref subcommand } => {
            project_362_engine::cli::vault::handle_vault_command(subcommand).await?;
            return Ok(());
        }
        Command::Install {
            ref url,
            ...
            project_362_engine::cli::install::handle_install(url, allow_untrusted).await?;
            return Ok(());
        }
        Command::Login => {
            project_362_engine::cli::login::handle_login_command().await?;
            return Ok(());
        }
        Command::Logout => {
            project_362_engine::cli::login::handle_logout_command().await?;
            return Ok(());
        }
        Command::Whoami => {
            project_362_engine::cli::login::handle_whoami_command().await?;
            return Ok(());
        }
        Command::Survey => {
            project_362_engine::cli::survey::handle_survey_command().await?;
            return Ok(());
        }
        Command::Auth { ref subcommand } => {
            project_362_engine::cli::auth::handle_auth_command(subcommand).await?;
            return Ok(());
        }
        Command::Db { ref subcommand } => {
            project_362_engine::cli::db::handle_db_command(subcommand).await?;
            return Ok(());
        }
        Command::Backup {
            ref subcommand,
            ...
            project_362_engine::cli::backup::handle_backup_command(subcommand, data_dir).await?;
            return Ok(());
        }
        Command::Export(ref args) => {
            project_362_engine::cli::export::handle_export_command(args).await?;
            return Ok(());
        }
        Command::Doctor => {
            eprintln!("project_362 doctor");
            eprintln!("=================");
            eprintln!();

            // permissions
            eprintln!("permissions:");
            let perms_ok = project_362_core::permissions::preflight_check(true, true);
            ...
            if port_ok {
                ...
                eprintln!("  port 3030: in use (project_362 may already be running)");
            ...
            if perms_ok && system_audio_ok && ffmpeg_ok && port_ok {
                ...
                eprintln!("some checks failed — fix the issues above before running project_362");
                std::process::exit(1);
            }
            return Ok(());
        }
        Command::Diagnose {
            ...
            let local_data_dir = get_base_dir(data_dir)?;
            project_362_engine::cli::diagnose::handle_diagnose_command(
                &local_data_dir,
                message.as_deref(),
                port,
                dry_run,
    ...
    if config.analytics_enabled && project_362_engine::analytics::telemetry_disabled_by_env() {
    ...
    project_362_connect::mdns::set_enabled(record_args.enable_mdns);
    ...
    project_362_engine::process_priority::set_background_priority();

    // Non-blocking update check — runs in background, prints banner if outdated
    tokio::spawn(async {
        project_362_engine::cli_reminder::check_for_updates().await;
    });

    // Periodic terminal nudge to install the desktop app (CLI-only).
    project_362_engine::cli_reminder::spawn();
    ...
    let _sentry_guard = if config.analytics_enabled {
        ...
        let guard = sentry::init((
            ...
            sentry::ClientOptions {
                ...
                server_name: Some("project-362-cli".into()),
                before_send: Some(std::sync::Arc::new(|mut event| {
                    ...
                    // Tauri-app filter in apps/project-362-app-tauri/src-tauri/
                    ...
                    let env_patterns = USER_ENV_PATTERNS.get_or_init(|| {
                        [
                            ...
                            // Port conflict — another project_362 instance is already bound
                            // (CLI-2J: 659 events / 649 users — user environment, not a bug)
                            r"you're likely already running project_362 instance",
        ...
        sentry::configure_scope(|scope| {
            ...
            // customers can set project_362_SUPPORT_ID to make standalone CLI
            ...
            let telemetry_context =
                project_362_engine::telemetry_context::TelemetryContext::from_env();
    ...
    // apps/project-362-app-tauri/src-tauri/src/main.rs.
    {
        // Write to the resolved data dir (honors --data-dir) so the crash log
        // sits next to project-362.log, and an embedder running with its own
        // --data-dir doesn't collide with the desktop app's
        // ~/.project-362/last-panic.log (the app runs its engine in-process and
        // owns that file).
        ...
        // Reuse the existing embedder attribution (project_362_EMBEDDER /
        // project_362_CUSTOMER_ID / ...) so the local crash record is identifiable
        ...
        let attribution = {
            use project_362_engine::telemetry_context::TelemetryContext;
    ...
    if !is_local_ipv4_port_free(config.port) {
        error!(
            "you're likely already running project_362 instance in a different environment, e.g. terminal/ide, close it and restart or use different port"
        );
    ...
    let _permission_monitor_handle = project_362_engine::permission_monitor::start();
    ...
    project_362_engine::schedule_monitor::reset_schedule_paused();
        ...
        Ok(mut manager) => {
            ...
            if !config.disable_timeline {
                ...
                manager.set_on_transcription_insert(std::sync::Arc::new(move |info| {
                    ...
                    rt.spawn(async move {
                        use project_362_engine::hot_frame_cache::HotAudio;
                        cache
                            .push_audio(HotAudio {
    ...
    if config.api_auth {
        info!("API auth enabled — run `project_362 auth token` to view your key");
    }

    // Standalone CLI users miss out on the chat/timeline UI — point them at the
    // desktop app. project_362_ANALYTICS_ID is only set when the Tauri app spawns
    // the engine, so its absence is a reliable "this is a bare CLI run" signal.
    if std::env::var("project_362_ANALYTICS_ID").is_err() {
        eprintln!();
        eprintln!("  tip: get the desktop app for chat, timeline, and search UI");
        eprintln!("       → https://project362.com/onboarding");
        eprintln!();
    }
    ...
    // project_362_API_KEY directly; desktop path overrides via
    // SCServer::cloud_token_handle after spawn.
    if let Ok(t) = std::env::var("project_362_API_KEY") {
        if !t.is_empty() {
    ...
    let encryption_requested =
        config.encrypt_secrets || project_362_secrets::is_encryption_requested(&local_data_dir);

    {
            ...
            if config.encrypt_secrets {
                match project_362_secrets::keychain::get_or_create_key() {
                    Some(k) => {
                        ...
                        if let Err(e) = project_362_secrets::mark_encryption_enabled(&local_data_dir)
                        {
                ...
                match project_362_secrets::keychain::get_key() {
                    project_362_secrets::keychain::KeyResult::Found(k) => {
        ...
        let secret_store_result =
            project_362_secrets::SecretStore::new(db.pool.clone(), secret_key).await;
        match secret_store_result {
            Ok(store) => {
                // Run startup permission sweep
                let fixed = project_362_secrets::fix_secret_file_permissions(&local_data_dir);
                if fixed > 0 {
                    info!("fixed permissions on {} credential files", fixed);
                }

                // Run legacy migration
                match project_362_secrets::migrate_legacy_secrets(&store, &local_data_dir).await {
                    Ok(report) => {
    ...
    let pi_executor = std::sync::Arc::new(
        project_362_core::agents::pi::PiExecutor::new(user_token.clone())
    ...
    // can override with project_362_EVENT_CLASSIFIER_URL.
    if config.enable_workflow_events {
        let classifier_url =
            std::env::var("project_362_EVENT_CLASSIFIER_URL").unwrap_or_else(|_| {
                project_362_engine::workflow_classifier::DEFAULT_CLASSIFIER_URL.to_string()
            });
        let token = user_token.clone().unwrap_or_default();
        let port = config.port;
        tokio::spawn(async move {
            project_362_engine::workflow_classifier::start_workflow_classifier(
                classifier_url,
                token,
                port,
    ...
    let mut agent_executors: std::collections::HashMap<
        String,
        std::sync::Arc<dyn project_362_core::agents::AgentExecutor>,
    > = std::collections::HashMap::new();
    agent_executors.insert("pi".to_string(), pi_executor.clone());

    // Create pipe store backed by the main SQLite DB
    let pipe_store: Option<std::sync::Arc<dyn project_362_core::pipes::PipeStore>> =
    ...
    let mcp_session_access = project_362_core::pipes::mcp_access::McpSessionAccessRegistry::new();
    ...
    pipe_manager.set_token_registry(std::sync::Arc::new(
        project_362_engine::pipe_permissions_middleware::DashMapTokenRegistry::new(
    ...
    // pipes that are still in "setup mode" (declared connections not paired).
    {
        ...
        let project_362_dir_for_check = local_data_dir.clone();
        pipe_manager.set_connection_check(std::sync::Arc::new(move |required| {
            ...
            Box::pin(async move {
                project_362_connect::missing_pipe_connections(ss.as_deref(), &dir, &required).await
            })
        }));
    }
    ...
    println!(
        ...
        if record_args.retention_days == 0 {
            ...
            match record_args.retention_mode {
                project_362_engine::retention::RetentionMode::Media => {
                    "media-only (keep transcripts)".to_string()
                }
                project_362_engine::retention::RetentionMode::Lean => {
                    "lean (keep text+memories)".to_string()
                }
                project_362_engine::retention::RetentionMode::All => "all (full delete)".to_string(),
            }
        }
    );
    ...
    println!(
        "\n{}",
        "check latest changes here: https://github.com/project-362/project-362/releases"
            .bright_blue()
            .italic()
    );
    ...
    if config.async_pii_redaction {
        ...
        //      from huggingface.co/project-362/pii-redactor under
        ...
        tokio::spawn(async move {
            ...
            info!(
                "fetching v45 phase 3 ONNX text redactor (~278 MB INT8 on first run, \
                 cached at ~/.project-362/models/v45_phase3_onnx/)"
            );
    ...
    if config.async_image_pii_redaction {
        ...
        use project_362_redact::{ImageRedactionPolicy, ImageRedactor};
        use std::sync::Arc;
        ...
        {
            if std::env::var_os("project_362_ENABLE_EXPERIMENTAL_RFDETR_MLX").is_some() {
                use project_362_redact::adapters::rfdetr_mlx::{RfdetrMlxConfig, RfdetrMlxRedactor};
                let mlx_cfg = RfdetrMlxConfig::default();
                // Mirrors the ONNX adapter: download once, verify SHA-256,
                // cache at ~/.project-362/models/rfdetr_v9.safetensors.
                ...
                tracing::info!(
                    "rfdetr-mlx disabled by default for CLI stability; \
                     set project_362_ENABLE_EXPERIMENTAL_RFDETR_MLX=1 to opt in"
                );
            }
        }
                ...
                Err(e) => {
                    ...
                    tracing::warn!(
                        "image-PII redaction enabled but couldn't load model; skipping: {e}. \
                         ...
                         rfdetr_v9.onnx at ~/.project-362/models/."
                    );
                }
            }
        }
