...
use crate::routes::search::is_project_362_app;
...
impl HotFrameCache {
    ...
    pub async fn warm_from_db(&self, db: &project_362_db::DatabaseManager, hours: i64) {
        let end = Utc::now();
            ...
            Ok(chunks) => {
                ...
                for frame_data in chunks.frames {
                    ...
                    for ocr_entry in &frame_data.ocr_entries {
                        ...
                        if is_project_362_app(&ocr_entry.app_name) {
                            continue;
                        }
                        let hot = HotFrame {
