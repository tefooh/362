...
//! project_362 cloud classifier. When a high-confidence workflow event is detected,
...
use reqwest::Client;
use project_362_events::{ActivityEntry, ClassifierResult, WorkflowEvent};
use serde_json::json;
...
/// Default endpoint for the event classifier — routes through the project_362
/// AI gateway, which forwards the `project-362-event-classifier` model to the
...
/// Override with the `project_362_EVENT_CLASSIFIER_URL` env var for self-host.
pub const DEFAULT_CLASSIFIER_URL: &str = "https://api.project-362.com";
...
/// Query the local project_362 `/search` API for recent activity across all content types.
...
/// Call the classifier through the project_362 gateway (default) or directly
/// against a self-hosted vLLM (`project_362_EVENT_CLASSIFIER_URL` override).
/// The model name `project-362-event-classifier` is recognized by both: vLLM
...
async fn classify(
    ...
    let body = json!({
        "model": "project-362-event-classifier",
        "messages": [
