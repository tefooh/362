...
//!   github.com/project-362/privacy-filter and pins TLS to the attested key,
...
/// `project_362_PRIVACY_FILTER_ENCLAVE` / `project_362_PRIVACY_FILTER_REPO`,
/// defaulting to the project-362-published enclave + its source repo so that
/// Tinfoil's measurement comparison ties this client to the exact open
/// source code at github.com/project-362/privacy-filter.
pub fn global() -> Arc<PrivacyFilter> {
    INSTANCE
        .get_or_init(|| {
            let enclave = std::env::var("project_362_PRIVACY_FILTER_ENCLAVE")
                .unwrap_or_else(|_| DEFAULT_ENCLAVE.to_string());
            let repo = std::env::var("project_362_PRIVACY_FILTER_REPO")
