...
//! the `project_362` CLI, `project_362 auth token`, the MCP bridge — must go
...
pub async fn resolve_api_auth_key(data_dir: &Path, settings_key: Option<&str>) -> Result<String> {
    ...
    let (key, source) = if let Ok(k) = std::env::var("project_362_API_KEY") {
        if !k.is_empty() {
            (k, "project_362_API_KEY env")
        } else {
    ...
    // reader (running server, MCP, `project_362 auth token` CLI) agrees on
    ...
    // rotating the user's API key (project-362-APP-9Z: 25 events / 18 users,
...
pub async fn set_api_auth_key(data_dir: &Path, key: &str) -> Result<()> {
    ...
    if let Some(home) = dirs::home_dir() {
        let _ = std::fs::remove_file(home.join(".project-362/auth.json"));
    }
...
async fn open_secret_store(data_dir: &Path) -> Result<project_362_secrets::SecretStore> {
    ...
    let key = if project_362_secrets::is_encryption_requested(data_dir) {
        match project_362_secrets::keychain::get_key() {
            project_362_secrets::keychain::KeyResult::Found(k) => Some(k),
            _ => None,
        }
    } else {
        None
    ...
    let store = project_362_secrets::SecretStore::open(&db_path.to_string_lossy(), key).await?;
    Ok(store)
}

fn read_legacy_auth_json() -> Option<String> {
    let home = dirs::home_dir()?;
    let content = std::fs::read_to_string(home.join(".project-362/auth.json")).ok()?;
...
pub async fn find_api_auth_key() -> Option<String> {
    if let Ok(k) = std::env::var("project_362_API_KEY") {
        if !k.is_empty() {
            return Some(k);
        }
    }
    ...
    // was right there.
    for var in ["project_362_LOCAL_API_KEY", "project_362_API_AUTH_KEY"] {
...
// ---------------------------------------------------------------------------
// Cloud auth token (Clerk JWT — the Bearer for api.project-362.com).
//
...
// `db.sqlite`) must read it there too — otherwise a standalone `project_362`
// run would lose cloud auth (STT, project-362-cloud pipes) once the app has
// migrated the token out of store.bin.
// ---------------------------------------------------------------------------

/// SecretStore key for the cloud auth token. MUST match the desktop app's
/// `auth_token` module (`apps/project-362-app-tauri/src-tauri/src/auth_token.rs`).
...
///   2. Fallback: `store.bin` `settings.user.token` — where the `project_362
...
///      login (e.g. `project_362 login` on a VPS, no app) leaves the engine
///      with no cloud token and cloud features like the `/v1/chat/completions`
///      proxy stay disabled — exactly the "project_362 on the go" case.
...
/// the `project_362 login` CLI writes). Skips app-managed encrypted stores
