...
use project_362_events::{send_event, subscribe_to_all_events, Event as Project362Event};
    ...
    let incoming = tokio::spawn(async move {
        while let Some(Ok(msg)) = receiver.next().await {
            if let Message::Text(t) = msg {
                if let Ok(event) = serde_json::from_str::<Project362Event>(&t) {
...
async fn handle_meeting_status_socket(
    ...
    let mut stream =
        project_362_events::subscribe_to_event::<MeetingStatusResponse>("meeting_status_changed");
    loop {
        tokio::select! {
            event = stream.next() => {
