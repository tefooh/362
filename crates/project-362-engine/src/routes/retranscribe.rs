...
use project_362_db::{AudioChunkInfo, NewMeetingTranscriptSegment};
...
pub struct MeetingRetranscribeRequest {
    /// Optional engine override: "deepgram", "project-362-cloud", "parakeet", etc.
    pub engine: Option<String>,
    /// Custom vocabulary for this re-transcription.
...
pub async fn retranscribe_handler(
    ...
    for chunk in &chunks {
        ...
        let (samples, sample_rate) =
            match project_362_audio::utils::ffmpeg::read_audio_from_file(file_path) {
                Ok(result) => result,
                Err(e) => {
    ...
    for batch in batches {
        ...
        for chunk in batch {
            ...
            match tokio::task::spawn_blocking(move || {
                project_362_audio::utils::ffmpeg::read_audio_from_file(StdPath::new(&path_owned))
            })
            .await
            {
                Ok(Ok((samples, sr))) => {
