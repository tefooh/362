...
use anyhow::Result;
use chrono::Utc;
use project_362_core::pipes::{PipeExecution, PipeStore, SchedulerState};
use project_362_db::DatabaseManager;
use std::sync::Arc;

/// SQLite-backed pipe store using the main project_362 database.
...
impl PipeStore for SqlitePipeStore {
    ...
    async fn set_execution_running(&self, id: i64, pid: Option<u32>) -> Result<()> {
        use project_362_db::write_queue::PipeBindValue;
...
mod tests {
    use super::*;
    use project_362_core::pipes::PipeStore;
    ...
    async fn test_create_execution() {
        ...
        let id = store
            .create_execution("my-pipe", "manual", "haiku", Some("project_362"))
            .await
            .unwrap();
        assert!(id > 0);
    }
