...
/// automation can set `project_362_DISABLE_TELEMETRY` to opt out explicitly.
/// This is the single chokepoint that keeps CI runs — the `project_362` CLI
...
pub fn telemetry_disabled_by_env() -> bool {
    ["project_362_DISABLE_TELEMETRY", "GITHUB_ACTIONS", "CI"]
        .iter()
        .any(|var| {
            std::env::var(var)
...
pub async fn capture_event(event: &str, properties: Value) {
    ...
    if let Some(obj) = props.as_object_mut() {
        ...
        obj.insert("$lib".to_string(), json!("project-362-engine"));
...
mod tests {
    ...
    const VARS: &[&str] = &["project_362_DISABLE_TELEMETRY", "GITHUB_ACTIONS", "CI"];
    ...
    fn telemetry_disabled_by_explicit_optout() {
        with_env(&[("project_362_DISABLE_TELEMETRY", "1")], || {
            assert!(telemetry_disabled_by_env())
        });
    }
