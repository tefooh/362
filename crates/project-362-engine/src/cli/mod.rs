...
fn default_audio_engine() -> CliAudioTranscriptionEngine {
    let tier = project_362_config::detect_tier();
    if matches!(tier, project_362_config::DeviceTier::Low) {
...
#[command(
    author,
    version,
    about = "project_362: power AI by everything you've seen, said or heard",
    long_about = None,
    name = "project_362"
)]
pub struct Cli {
...
pub enum Command {
    ...
    /// Show project_362 status (running state, data stats)
    Status {
        /// Output format
        #[arg(long, default_value_t = false)]
        json: bool,
        /// Data directory. Default to $HOME/.project-362
    ...
    /// (no daemon required — opens `~/.project-362/db.sqlite` read-side
    /// via WAL while sp may be writing).
    Search(SearchArgs),

    /// Enterprise: query teammates' screen + audio history via
    /// `project-362.com/api/enterprise/v1/*`. Admin-only — needs a
    /// `team_api_token` minted at https://project-362.com/enterprise?tab=tokens.
    Team {
        #[command(subcommand)]
    ...
    /// Wire an external agent (OpenClaw, Hermes, Claude) to project_362
    Agent {
        #[command(subcommand)]
        subcommand: agent::AgentCommand,
    },

    /// Run project_362 as a background service that starts at boot + stays up
    ...
    /// Vault: encrypt/lock all project_362 data at rest
    Vault {
        #[command(subcommand)]
    ...
    Install {
        /// Manifest URL (HTTPS, JSON). Defaults to the project_362 starter bundle.
        #[arg(default_value = "https://project-362.com/start.json")]
        url: String,
    ...
    /// Authenticate with project_362 cloud
    Login,

    /// Sign out of project_362 cloud (clears the auth token from store.bin)
    Logout,

    /// Show current auth status
    Whoami,

    /// Open the project_362 survey in your browser
    Survey,

    /// Check system readiness (permissions, ffmpeg, etc.)
    Doctor,

    /// Collect recent logs + system info and send them to project_362 support,
    ...
    /// `~/.project-362/db.sqlite` directly — no daemon required.
...
pub struct ExportArgs {
    ...
    /// (see `project_362 search` or the app's meetings list). Mutually
...
pub struct RecordArgs {
    ...
    /// Enable debug logging for project_362 modules
    #[arg(long)]
    pub debug: bool,
    ...
    /// `rfdetr_v11.onnx` at `~/.project-362/models/` and the binary
    ...
    /// `tinfoil` (project-362-hosted confidential-compute enclave on
    ...
    #[arg(long, env = "project_362_ENABLE_MDNS", default_value_t = false)]
    pub enable_mdns: bool,

    /// API token for cloud sync
    #[arg(long, env = "project_362_SYNC_TOKEN")]
    pub sync_token: Option<String>,

    /// Password for encrypting synced data
    #[arg(long, env = "project_362_SYNC_PASSWORD")]
    ...
    /// ~/.project-362 data into a remote LLM (passwords, keys, secrets often
    ...
    /// capture on-screen text. Useful when piping ~/.project-362 data into a
    ...
    /// requests must include Authorization: Bearer <project_362_API_KEY>.
...
impl RecordArgSources {
    pub fn from_cli_matches(matches: &ArgMatches) -> Self {
        ...
        Self {
            ...
            project_362_aec_enabled: from_command_line(record, "project_362_aec_enabled"),
...
fn parse_schedule_rule(s: &str) -> Result<project_362_config::ScheduleRule, String> {
    ...
    Ok(project_362_config::ScheduleRule {
        day_of_week,
        start_time,
        end_time,
        record_mode,
...
impl RecordArgs {
    ...
    pub fn to_recording_settings(&self) -> project_362_config::RecordingSettings {
        ...
        let aec_mode = if self.project_362_aec_enabled {
            project_362_config::AecMode::Project362
        } else if self.windows_input_aec_enabled {
            project_362_config::AecMode::Windows
        } else if self.macos_input_vpio_enabled {
            project_362_config::AecMode::Macos
        } else {
            project_362_config::AecMode::Off
        };
        ...
        project_362_config::RecordingSettings {
            ...
            windows_input_aec_enabled: aec_mode == project_362_config::AecMode::Windows,
            macos_input_vpio_enabled: aec_mode == project_362_config::AecMode::Macos,
            project_362_aec_enabled: aec_mode == project_362_config::AecMode::Project362,
            aec_mode,
            ...
            ..project_362_config::RecordingSettings::default()
        }
    }
    ...
    pub async fn into_recording_config(
        ...
        let cli_encryption_ready = if self.encrypt_secrets {
            match project_362_secrets::keychain::get_or_create_key() {
                Some(_) => {
                    project_362_secrets::mark_encryption_enabled(&data_dir)?;
                    true
                }
                None => {
                    tracing::warn!(
        ...
        // or cloud features (STT, project-362-cloud pipes) silently lose auth
        ...
        if settings.device_tier.is_none() {
            ...
            let existing = project_362_config::load_toml(&config_path).ok();
            ...
            if has_tier {
                ...
                if is_fresh {
                    project_362_config::apply_tier_defaults(&mut settings, tier);
        ...
        let tier = settings
            ...
            .unwrap_or_else(project_362_config::detect_tier);
        if project_362_config::is_engine_unsafe(&settings.audio_transcription_engine, tier) {
            let safe = project_362_config::best_engine_for_platform(tier);
            eprintln!(
        ...
        if self.listen_on_lan && !self.api_auth {
            tracing::warn!(
                "--listen-on-lan was set but --api-auth=false — forcing api_auth on for safety. Use `project_362 auth token` to view your key."
            );
        }
        if config.api_auth {
    ...
    fn apply_explicit_overrides(
        &self,
        settings: &mut project_362_config::RecordingSettings,
        sources: &RecordArgSources,
    ) {
        ...
        if sources.project_362_aec_enabled && self.project_362_aec_enabled {
            settings.aec_mode = project_362_config::AecMode::Project362;
            settings.project_362_aec_enabled = true;
            ...
            settings.aec_mode = project_362_config::AecMode::Windows;
            settings.project_362_aec_enabled = false;
            settings.windows_input_aec_enabled = true;
            settings.macos_input_vpio_enabled = false;
        } else if sources.macos_input_vpio_enabled && self.macos_input_vpio_enabled {
            settings.aec_mode = project_362_config::AecMode::Macos;
            settings.project_362_aec_enabled = false;
            settings.windows_input_aec_enabled = false;
...
fn load_recording_settings_from_store(
    data_dir: &Path,
) -> anyhow::Result<Option<project_362_config::RecordingSettings>> {
...
pub enum PipeCommand {
    ...
    Publish {
        /// Pipe name (directory name under ~/.project-362/pipes/)
        name: String,
    },
...
pub enum ModelCommand {
    ...
    Create {
        ...
        id: String,
        /// Provider: openai | anthropic | native-ollama | custom | project-362-cloud
        #[arg(long)]
        provider: String,
...
pub enum VaultCommand {
    ...
    Setup {},
    /// Lock the vault (encrypt all data; stop project_362 first)
    Lock {
        /// Port project_362 listens on. `vault lock` refuses to run while a
        /// daemon is detected here — locking encrypts the DB in place and would
        /// corrupt it underneath a live daemon, so stop project_362 first.
        #[arg(short = 'p', long, default_value_t = 3030)]
        port: u16,
    },
...
pub enum DbCommand {
    ...
    /// and atomically swaps in the recovered db. Refuses to run while project_362
    /// is open (the desktop app would race the swap).
    Recover {
        /// Run even if the project_362 HTTP server is reachable. Dangerous —
    ...
    Cleanup {
        ...
        #[arg(long)]
        apply: bool,
        /// Run even if the project_362 HTTP server is reachable.
        #[arg(long)]
        force: bool,
    },
    /// Force-clear the .db_recovery.lock file (escape hatch if a previous
    /// `project_362 db ...` run was killed without releasing the lock).
    Unlock {
...
/// shape the API returns — `project_362 search` and `curl /search` are
...
pub struct SearchArgs {
    ...
    /// Data directory. Default `$HOME/.project-362`.
    #[arg(long, value_hint = ValueHint::DirPath)]
    pub data_dir: Option<String>,
...
/// Mirrors the `project-362-team` skill 1:1 — same endpoints, same vocabulary.
/// All three variants hit `https://project-362.com/api/enterprise/v1/*` directly
/// with the admin's `team_api_token` from `~/.project-362/enterprise.json`
/// (or `project_362_TEAM_API_TOKEN` env override). No daemon needed.
...
pub struct TeamSearchArgs {
    ...
    /// Restrict to one device — get the id from `project_362 team devices`.
    #[arg(long)]
...
pub enum McpCommand {
    ...
    Setup {
        /// Directory to save MCP files (default: $HOME/.project-362/mcp)
        #[arg(long, value_hint = ValueHint::DirPath)]
...
pub enum SyncCommand {
    ...
    /// Sync ~/.project-362 to a remote SSH server (SFTP, no cloud account)
    Remote {
        #[command(subcommand)]
...
/// SSH/SFTP-based sync of `~/.project-362` to a remote server.
///
/// No cloud account or project-362-cloud dependency — pushes the entire data
...
pub enum RemoteSyncCommand {
    ...
    /// Push `~/.project-362/` to the remote once
    Now {
        #[command(flatten)]
        cfg: RemoteSyncArgs,
        /// Override the local data directory (default: $HOME/.project-362)
        #[arg(long)]
        data_dir: Option<String>,
    },
...
/// Shared connection flags for `project_362 sync remote {test,now}`.
///
/// All values can also come from env vars: project_362_REMOTE_HOST,
/// project_362_REMOTE_USER, project_362_REMOTE_KEY, project_362_REMOTE_PATH.
#[derive(clap::Args, Debug)]
pub struct RemoteSyncArgs {
    /// Remote host (IP or DNS, e.g. "myserver.tail-scale.ts.net")
    #[arg(long, env = "project_362_REMOTE_HOST")]
    pub host: String,
    /// SSH user
    #[arg(long, env = "project_362_REMOTE_USER")]
    pub user: String,
    /// Path to SSH private key (e.g. ~/.ssh/id_ed25519)
    #[arg(long, env = "project_362_REMOTE_KEY")]
    pub key_path: String,
    /// Absolute path on the remote where ~/.project-362/ should land
    #[arg(long, env = "project_362_REMOTE_PATH")]
    ...
    /// way to put a huge ~/.project-362 on a small remote.
    #[arg(long, default_value_t = false)]
    pub no_media: bool,
    /// Exclude paths matching a glob (repeatable, `*` wildcards). Also reads
    /// `<data-dir>/.project-362ignore` (one glob per line, `#` comments).
    #[arg(long = "exclude")]
...
pub fn get_or_create_machine_id(override_id: Option<String>) -> String {
    ...
    project_362_core::sync::get_or_create_machine_id()
}
...
mod tests {
    ...
    fn test_pause_on_drm_content_default_false() {
        let cli = Cli::try_parse_from(["project_362", "record"]).unwrap();
        match cli.command {
            Command::Record(args) => {
    ...
    fn test_pause_on_drm_content_flag_sets_true() {
        let cli = Cli::try_parse_from(["project_362", "record", "--pause-on-drm-content"]).unwrap();
        match cli.command {
            Command::Record(args) => {
    ...
    fn test_keep_computer_awake_flag_flows_to_recording_settings() {
        let cli = Cli::try_parse_from(["project_362", "record", "--keep-computer-awake"]).unwrap();
        match cli.command {
            Command::Record(args) => {
                let settings = args.to_recording_settings();
                assert!(
    ...
    fn test_schedule_rule_rejects_bad_input() {
        ...
        assert!(
            Cli::try_parse_from(["project_362", "record", "--schedule-rule", "0,9am,18:00,all"])
                .is_err(),
    ...
    fn test_survey_command_parses() {
        let cli = Cli::try_parse_from(["project_362", "survey"]).unwrap();
        match cli.command {
            Command::Survey => {}
    ...
    fn test_profile_command_parses_with_defaults() {
        let cli = Cli::try_parse_from(["project_362", "profile"]).unwrap();
        match cli.command {
    ...
    fn test_profile_command_parses_port_and_json() {
        let cli =
            Cli::try_parse_from(["project_362", "profile", "--port", "4040", "--json"]).unwrap();
        match cli.command {
            Command::Profile { json, port } => {
                assert!(json);
                assert_eq!(port, 4040);
    ...
    fn test_record_arg_sources_only_mark_explicit_cli_args() {
        let matches = Cli::command()
            .try_get_matches_from(["project_362", "record", "--port", "4040"])
            .unwrap();
    ...
    fn test_explicit_cli_overrides_preserve_app_settings_for_absent_defaults() {
        let cli =
            Cli::try_parse_from(["project_362", "record", "--port", "4040", "--disable-audio"])
                .unwrap();
        let sources = record_sources(["project_362", "record", "--port", "4040", "--disable-audio"]);
        let mut settings = project_362_config::RecordingSettings {
            port: 3030,
            disable_audio: false,
    ...
    fn test_no_audio_flags_preserve_persisted_disable_audio() {
        ...
        let args = ["project_362", "record", "--port", "4040"];
        let cli = Cli::try_parse_from(args).unwrap();
        let sources = record_sources(args);
        let mut settings = project_362_config::RecordingSettings {
            disable_audio: true,
    ...
    fn test_monitor_id_flag_enables_vision_over_persisted_disable() {
        ...
        let args = ["project_362", "record", "--monitor-id", "5"];
        let cli = Cli::try_parse_from(args).unwrap();
        let sources = record_sources(args);
        let mut settings = project_362_config::RecordingSettings {
            disable_vision: true,
    ...
    fn test_default_true_bools_default_to_true_without_flag() {
        let args = record_args(["project_362", "record"]);
    ...
    fn test_default_true_bools_accept_space_separated_value() {
        let args = record_args(["project_362", "record", "--use-pii-removal", "false"]);
        assert!(
            !args.use_pii_removal,
            "--use-pii-removal false must disable"
        );
    ...
    fn test_eq_false_registers_as_cli_source_and_overrides_persisted_settings() {
        let args = ["project_362", "record", "--use-pii-removal=false"];
        let cli = Cli::try_parse_from(args).unwrap();
        let sources = record_sources(args);
        assert!(
