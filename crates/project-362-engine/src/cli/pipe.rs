...
use project_362_core::pipes::PipeManager;
    ...
    match command {
        PipeCommand::List { json } => {
            ...
            if *json {
                ...
                println!("\nhint: create one at ~/.project-362/pipes/my-pipe/pipe.md");
            } else {
                println!(
                    "{:<20} {:<10} {:<12} {:<20}",
...
pub fn api_base_url() -> String {
    std::env::var("project_362_API_BASE_URL")
        .unwrap_or_else(|_| "https://project-362.com".to_string())
}

/// Get the auth token, checking in order:
/// 1. project_362_API_KEY env var
/// 2. ~/.project-362/store.bin (settings.user.token — written by desktop app or `project_362 login`)
pub fn get_auth_token() -> Option<String> {
    if let Ok(key) = std::env::var("project_362_API_KEY") {
        return Some(key);
    }
...
async fn handle_publish_command(name: &str, pipes_dir: &std::path::Path) -> anyhow::Result<()> {
    ...
    if !pipe_md_path.exists() {
        anyhow::bail!(
            "pipe.md not found at {}\nhint: make sure the pipe '{}' exists in ~/.project-362/pipes/",
    ...
    let token = get_auth_token().ok_or_else(|| {
        anyhow::anyhow!(
            "no auth token found. set project_362_API_KEY env var or create ~/.project-362/auth.json"
        )
    })?;
...
async fn handle_info_command(slug: &str) -> anyhow::Result<()> {
    ...
    println!();
    println!("install with: project_362 pipe install {}", slug);
...
fn model_list(json: bool) -> anyhow::Result<()> {
    ...
    if presets.is_empty() {
        ...
        println!(
            "\nhint: create one with `project_362 pipe models create <id> --provider … --model …`"
        );
        return Ok(());
    }
...
async fn handle_set_preset_command(
    ...
    if manager.get_pipe(name).await.is_none() {
        anyhow::bail!(
            "pipe '{}' not found — install it first with `project_362 pipe install <source>`",
            name
        );
    }
