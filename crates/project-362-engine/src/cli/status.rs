...
pub async fn handle_status_command(
    ...
    // db.sqlite lives at the top of the project_362 dir; the `data/` subdir
    ...
    let (frame_count, audio_count, last_timestamp) = if db_path.exists() {
        match project_362_db::DatabaseManager::new(&db_path.to_string_lossy(), Default::default())
            .await
        {
            Ok(db) => {
    ...
    if json {
        ...
        println!("project_362: {} {} (port {})", status_icon, status, port);
