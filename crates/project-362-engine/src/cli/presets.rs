...
//!
//! Reads/writes presets in `~/.project-362/store.bin` (plain JSON or the
...
//! at the file level — same trade-off as `project_362 login`.
...
/// stored on disk and consumed by `resolve_preset` in `project-362-core`.
...
impl Provider {
    pub fn parse(s: &str) -> Result<Self> {
        match s {
            ...
            "project-362-cloud" | "pi" => Ok(Provider::Project362Cloud),
            "openai-chatgpt" => Ok(Provider::OpenAiChatGpt),
            other => bail!(
                "unknown provider '{}'. Valid: openai, anthropic, native-ollama, custom, project-362-cloud",
                other
            ),
        }
    }
    ...
    pub fn as_canonical(self) -> &'static str {
        match self {
            ...
            Provider::Project362Cloud => "project-362-cloud",
    ...
    fn forbids_url(self) -> bool {
        matches!(self, Provider::Project362Cloud)
    }

    fn forbids_api_key(self) -> bool {
        matches!(
            self,
            Provider::NativeOllama | Provider::Project362Cloud | Provider::OpenAiChatGpt
        )
    }
}
...
fn validate_provider_combo(
    ...
    if provider.forbids_api_key() && api_key.map(|s| !s.is_empty()).unwrap_or(false) {
        let reason = match provider {
            ...
            Provider::Project362Cloud => "cloud uses your login token (run `project_362 login`)",
...
/// Scan `~/.project-362/pipes/*/pipe.md` and return the names of pipes whose
...
mod tests {
    ...
    fn provider_combos() {
        ...
        assert!(validate_provider_combo(Provider::Project362Cloud, None, None).is_ok());
        assert!(
            validate_provider_combo(Provider::Project362Cloud, Some("http://x"), None).is_err()
        );
        assert!(validate_provider_combo(Provider::Project362Cloud, None, Some("k")).is_err());
