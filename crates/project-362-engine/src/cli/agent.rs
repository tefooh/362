...
//! `project_362 agent setup` — make an external agent (OpenClaw, Hermes, Claude)
//! aware of project_362 in one command: install the project_362 skill(s) into the
//! agent's skills directory and register the project_362 MCP server in its config.
//!
//! The skills are host-aware: pass `--api-url` (e.g. when the agent runs on a
//! VPS that holds a synced copy of your project_362 data) and the written
...
// Canonical skill sources — single source of truth lives in project-362-core
...
const API_SKILL_MD: &str =
    include_str!("../../../project-362-core/assets/skills/project-362-api/SKILL.md");
const CLI_SKILL_MD: &str =
    include_str!("../../../project-362-core/assets/skills/project-362-cli/SKILL.md");

#[derive(clap::Subcommand, Debug)]
pub enum AgentCommand {
    /// Install the project_362 skill + register the MCP server into an agent.
    Setup {
        /// Which agent to wire up.
        #[arg(value_parser = ["openclaw", "hermes", "claude-code", "claude-desktop", "codex", "cursor", "windsurf"])]
        target: String,
        /// project_362 REST API base URL the skill + MCP should target.
...
fn write_skill(skills_dir: &Path, name: &str, md: &str, api_url: &str) -> Result<PathBuf> {
    ...
    // target host so an off-box agent hits the right project-362.
...
fn setup(target: &str, api_url: &str) -> Result<()> {
    ...
    println!("wiring project_362 → {} (api: {})", l.name, api_url);

    if let Some(skills_dir) = &l.skills_dir {
        let api_path = write_skill(skills_dir, "project-362-api", API_SKILL_MD, api_url)?;
        let cli_path = write_skill(skills_dir, "project-362-cli", CLI_SKILL_MD, api_url)?;
    ...
    if remote {
        println!(
            "\nnote: the agent will query project_362 at {api_url}. make sure that host\n      is reachable from the agent (run `project_362 record` there over your\n      synced data, or expose the API via a tunnel)."
        );
    }
    Ok(())
}

/// Idempotently add the `project_362` server to a JSON MCP config (OpenClaw,
...
fn merge_mcp_json(path: &Path, remote: bool, api_url: &str) -> Result<()> {
    ...
    let mut entry = json!({ "command": "npx", "args": ["-y", "project-362-mcp@latest"] });
    if remote {
        entry["env"] = json!({ "project_362_API_URL": api_url });
    }
    ...
    servers.insert("project_362".to_string(), entry);
    std::fs::write(path, serde_json::to_string_pretty(&root)? + "\n")?;
    println!("  ✓ mcp   {}", path.display());
    Ok(())
}

/// Add the `project_362` server to a YAML MCP config (Hermes). We don't pull a
...
fn merge_mcp_yaml(path: &Path, remote: bool, api_url: &str) -> Result<()> {
    ...
    let env_block = if remote {
        format!("\n    env:\n      project_362_API_URL: {api_url}")
    } else {
        String::new()
    };
    let server = format!(
        "  project_362:\n    command: npx\n    args:\n      - \"-y\"\n      - project-362-mcp@latest{env_block}\n"
    );
    let existing = std::fs::read_to_string(path).unwrap_or_default();

    if existing.contains("project-362-mcp") {
        println!(
            "  • {} already references project-362-mcp; left as-is",
            path.display()
        );
        return Ok(());
    }
...
/// Add the `project_362` server to a TOML MCP config (Codex). No TOML lib —
/// append a `[mcp_servers.project-362]` table if absent, preserving the rest of
...
fn merge_mcp_toml(path: &Path, remote: bool, api_url: &str) -> Result<()> {
    ...
    let env_block = if remote {
        format!("\n[mcp_servers.project-362.env]\nproject_362_API_URL = \"{api_url}\"\n")
    } else {
        String::new()
    };
    let block = format!(
        "[mcp_servers.project-362]\ncommand = \"npx\"\nargs = [\"-y\", \"project-362-mcp@latest\"]\n{env_block}"
    );
    let existing = std::fs::read_to_string(path).unwrap_or_default();
    if existing.contains("[mcp_servers.project-362]") {
        println!(
            "  • {} already has [mcp_servers.project-362]; left as-is",
            path.display()
        );
        return Ok(());
    }
...
mod tests {
    ...
    fn test_merge_mcp_json_fresh_and_idempotent() {
        ...
        assert_eq!(v["mcpServers"]["project_362"]["command"], "npx");
        assert!(v["mcpServers"]["project_362"]["env"].is_null());
        ...
        assert_eq!(
            v["mcpServers"]["project_362"]["env"]["project_362_API_URL"],
            "http://box:3030"
        );
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn test_merge_mcp_toml() {
        ...
        assert!(s.contains("[mcp_servers.project-362]"));
        assert!(s.contains("project_362_API_URL = \"http://box:3030\""));

        // idempotent: no duplicate table.
        merge_mcp_toml(&path, true, "http://box:3030").unwrap();
        let s2 = std::fs::read_to_string(&path).unwrap();
        assert_eq!(s2.matches("[mcp_servers.project-362]").count(), 1);
        let _ = std::fs::remove_dir_all(&dir);
    }
}
