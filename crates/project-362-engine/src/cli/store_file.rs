...
pub fn store_path() -> PathBuf {
    project_362_core::paths::default_project_362_data_dir().join("store.bin")
}
...
fn write_store_to_with_key(
    ...
    let bytes = if encrypt {
        ...
        let ciphertext = project_362_vault::crypto::encrypt_small(&serialized, &key)
...
fn store_encryption_key() -> Result<[u8; 32]> {
    match project_362_secrets::keychain::get_key() {
        ...
        project_362_secrets::keychain::KeyResult::AccessDenied => bail!(
            "store.bin is encrypted, but keychain access was denied. Grant project_362 keychain access or disable store encryption in the app, then try again."
        ),
        project_362_secrets::keychain::KeyResult::NotFound => bail!(
            "store.bin is encrypted, but the project_362 keychain key was not found. Open the app once or disable store encryption, then try again."
        ),
        project_362_secrets::keychain::KeyResult::Unavailable => bail!(
