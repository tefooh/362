...
use project_362_connect::remote_sync;
...
pub async fn start_sync_service(
    ...
    let token = args.sync_token.as_ref().ok_or_else(|| {
        anyhow::anyhow!("--sync-token or project_362_SYNC_TOKEN required for sync")
    })?;

    let password = args.sync_password.as_ref().ok_or_else(|| {
        anyhow::anyhow!("--sync-password or project_362_SYNC_PASSWORD required for sync")
    })?;
    ...
    let provider = Arc::new(Project362SyncProvider::new(db, machine_id));
...
/// Load extra exclude globs from `<data-dir>/.project-362ignore`
/// (one glob per line; blank lines and `#` comments ignored).
fn load_project_362ignore(dir: &std::path::Path) -> Vec<String> {
    match std::fs::read_to_string(dir.join(".project-362ignore")) {
        Ok(content) => content
            .lines()
            .map(|l| l.trim())
...
/// Handle `project_362 sync remote ...` — SFTP push of ~/.project-362 to an SSH server.
async fn handle_remote_sync_command(command: &RemoteSyncCommand) -> anyhow::Result<()> {
    match command {
        ...
        RemoteSyncCommand::Now { cfg, data_dir } => {
            ...
            excludes.extend(load_project_362ignore(&dir));
