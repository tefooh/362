...
pub async fn handle_connection_command(command: &ConnectionCommand) -> anyhow::Result<()> {
    ...
    let secret_store = if db_path.exists() {
        ...
        match project_362_secrets::SecretStore::open(&db_path.to_string_lossy(), None).await {
    ...
    let cm = ConnectionManager::new(project_362_dir.clone(), secret_store);
    ...
    match command {
        ...
        ConnectionCommand::Get { id, json: use_json } => {
            ...
            if let Some(creds) = has_creds {
                ...
                println!("\nhint: project_362 connection set {} key=value ...", id);
            }
        }
