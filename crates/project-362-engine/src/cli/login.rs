...
/// Handle `project_362 login` — opens browser, polls server for token.
pub async fn handle_login_command() -> anyhow::Result<()> {
    // Check if already logged in
    if let Some(token) = super::pipe::get_auth_token() {
        if !token.is_empty() {
            println!("already logged in. use `project_362 whoami` to check status.");
            return Ok(());
        }
    }
    ...
    loop {
        ...
        match status {
            "complete" => {
                ...
                // Save to ~/.project-362/store.bin (same file the desktop app uses)
                let mut store: Value = super::store_file::read_store()?;

                // Write to top-level `settings.user` — the canonical path the
                // desktop app reads (see apps/project-362-app-tauri/src-tauri/src/store.rs
...
/// Handle `project_362 logout` — clear cloud auth from store.bin.
///
...
pub async fn handle_logout_command() -> anyhow::Result<()> {
    ...
    println!();
    println!("  run `project_362 login` to sign back in");
    println!();

    Ok(())
}

/// Handle `project_362 whoami` — show current auth status.
pub async fn handle_whoami_command() -> anyhow::Result<()> {
    ...
    match token {
        Some(t) if !t.is_empty() => {
            ...
            let source = if std::env::var("project_362_API_KEY").is_ok() {
                "project_362_API_KEY env var"
            } else {
                "~/.project-362/store.bin"
            };
        ...
        _ => {
            ...
            println!();
            println!("  run `project_362 login` to authenticate");
            println!();
        }
    }
