...
//! `project_362 db ...` — corruption recovery + storage cleanup.
//!
//! ## Safety model
//!
//! These commands rewrite or delete files inside `~/.project-362/`. A racing
...
//!
//!   * `~/.project-362/.db_recovery.lock` — JSON `{pid, host, started_at, op}`
...
//!     (`apps/project-362-app-tauri/src-tauri/src/main.rs`); env var
//!     `project_362_IGNORE_DB_LOCK=1` is the escape hatch.
//!
...
//! 3. Same host, PID alive: refuse, point at `project_362 db unlock`.
...
if you're sure no `project_362 db ...` is running, run `project_362 db unlock --force`.",
                        path.display(),
                    );
                }
                let _ = fs::remove_file(&path);
            ...
            LockState::HeldHere { pid, op: other } => {
                bail!(
                    "another db op is running: {} (pid {}). wait for it, or `project_362 db unlock --force` if you're sure it's stuck.",
                    other,
                    pid,
                );
            }
...
will not auto-clear cross-host locks. resolve manually if needed.",
        ...
        let mut file = OpenOptions::new()
            ...
            .with_context(|| {
                format!(
                    "couldn't acquire lock at {}. another `project_362 db ...` may have raced us.",
                    path.display()
                )
            })?;
    ...
    fn install_signal_handlers(path: &Path, stop: Arc<AtomicBool>) {
        ...
        tokio::spawn(async move {
            ...
            let _ = fs::remove_file(&path);
            eprintln!("\nproject_362: caught signal, lock released. exiting.");
...
fn ensure_app_quit(force: bool) -> Result<()> {
    if !force && server_is_running() {
        bail!(
            "project_362 is currently running (port 3030 is open). quit the app from the menubar first, or pass --force to override (only safe if you know nothing is writing to the db)."
        );
    }
    Ok(())
}
...
fn ensure_disk_space(data_dir: &Path, source_size: u64) -> Result<()> {
    ...
    if let Some(avail) = available {
        if avail < needed {
            bail!(
                "not enough disk space: need ~{} MB free, have {} MB. run `project_362 db cleanup` first to reclaim old artifacts.",
                needed / 1_048_576,
                avail / 1_048_576,
            );
        }
...
fn integrity_check(db_path: &Path) -> Result<()> {
    ...
    if result == "ok" {
        ...
        eprintln!("\n  next step: quit the project_362 app, then run `project_362 db recover`");
        std::process::exit(1)
    }
}
...
async fn recover(data_dir: &Path, force: bool) -> Result<()> {
    ...
    println!();
    println!("    next: start project_362 — it will rebuild WAL/SHM on first open.");
    println!("    once you've confirmed everything works, run:");
    println!("        project_362 db cleanup --apply");
...
fn unlock(data_dir: &Path, force: bool) -> Result<()> {
    ...
    if !safe && !force {
        bail!(
            "lock looks live ({state:?}). pass --force to remove anyway. you should only do this if you're certain no `project_362 db ...` is actually running."
        );
    }
