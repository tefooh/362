...
//! `project_362 service {install,uninstall,status}` — run project_362 as a
//! background service that starts at boot and stays up (Restart=always). This
//! is what makes a headless box (a VPS, a Mac mini) keep project_362 running
...
pub enum ServiceCommand {
    /// Install + enable a service that runs project_362 at boot, always-on.
    Install {
        /// Args appended to `project_362 record`. Default is serve-only (no
...
const SERVICE_NAME: &str = "project_362";
const LAUNCHD_LABEL: &str = "com.project-362.agent";
...
fn systemd_unit(exec_start: &str, path_env: &str) -> String {
    format!(
        "[Unit]\n\
         Description=project_362 — always-on capture/serve\n\
...
fn project_362_exe() -> Result<String> {
    Ok(std::env::current_exe()
        .context("could not resolve the project_362 binary path")?
        .to_string_lossy()
        .into_owned())
}
...
fn install(record_args: &str) -> Result<()> {
    ...
    println!("  ✓ project_362 service enabled + started — runs at boot, restarts on crash");
...
fn install(record_args: &str) -> Result<()> {
    ...
    println!("  ✓ project_362 launch agent loaded — runs at login, restarts on crash");
    Ok(())
}
...
fn uninstall() -> Result<()> {
    ...
    println!("  ✓ project_362 service removed");
    Ok(())
}
...
fn uninstall() -> Result<()> {
    ...
    println!("  ✓ project_362 launch agent removed");
    Ok(())
}
...
mod tests {
    ...
    fn test_systemd_unit() {
        let u = systemd_unit(
            "/usr/local/bin/project-362 record --disable-vision",
            "/usr/bin",
        );
        assert!(u.contains("ExecStart=/usr/local/bin/project-362 record --disable-vision"));
    ...
    fn test_launchd_plist() {
        let p = launchd_plist(&[
            "/Applications/x/project-362".to_string(),
            "record".to_string(),
            "--disable-audio".to_string(),
        ]);
        assert!(p.contains("<key>Label</key>"));
        assert!(p.contains("com.project-362.agent"));
