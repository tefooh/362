...
use project_362_db::{DatabaseManager, FrameData, OCREntry};
...
impl From<project_362_db::AudioEntry> for AudioEntry {
    fn from(db_entry: project_362_db::AudioEntry) -> Self {
        Self {
...
impl FrameCache {
    pub async fn new(project_362_dir: PathBuf, db: Arc<DatabaseManager>) -> Result<Self> {
        let cache_config = CacheConfig {
            cache_dir: cache_dir().unwrap().join("project_362").join("frames"),
            ..Default::default()
        };
