...
use project_362_audio::audio_manager::AudioManager;
use project_362_core::sync::SyncServiceHandle;
...
pub struct AppState {
    ...
    pub audio_metrics: Arc<project_362_audio::metrics::AudioPipelineMetrics>,
    ...
    pub pipe_permissions:
        Arc<DashMap<String, Arc<project_362_core::pipes::permissions::PipePermissions>>>,
    ...
    pub vault: project_362_vault::VaultManager,
    ...
    pub browser_registry: Arc<project_362_connect::connections::browser::BrowserRegistry>,
    ...
    pub owned_browser: Arc<project_362_connect::connections::browser::OwnedBrowser>,
    /// When true, non-localhost requests require Authorization: Bearer <api_key>
    pub api_auth: bool,
    /// The API key to validate against (from project_362_API_KEY or auth.json)
    pub api_auth_key: Option<String>,
    /// Cloud JWT (Clerk) used to authenticate proxied requests to api.project-362.com.
    ...
    pub secret_store: Option<Arc<project_362_secrets::SecretStore>>,
...
pub struct SCServer {
    ...
    pub mcp_session_access: Option<project_362_core::pipes::mcp_access::McpSessionAccessRegistry>,
    ...
    pub owned_browser: Option<Arc<project_362_connect::connections::browser::OwnedBrowser>>,
...
impl SCServer {
    ...
    pub fn new(
        ...
        SCServer {
            ...
            video_quality,
            pipe_manager: None,
            vision_metrics: Arc::new(project_362_screen::PipelineMetrics::new()),
            audio_metrics,
    ...
    /// to api.project-362.com. Safe to call before or after `start()` — the route
    ...
    pub fn with_mcp_session_access(
        mut self,
        registry: project_362_core::pipes::mcp_access::McpSessionAccessRegistry,
    ) -> Self {
    ...
    pub async fn start(self) -> Result<(), std::io::Error> {
        ...
        if self.advertise_mdns {
            if let Err(e) = project_362_connect::mdns::advertise(self.addr.port()) {
    ...
    pub async fn create_router(&self) -> Router {
        ...
        let app_state = Arc::new(AppState {
            ...
            vault: project_362_vault::VaultManager::new(self.project_362_dir.clone()),
            manual_meeting: self
                .manual_meeting
                .clone()
                .unwrap_or_else(|| Arc::new(tokio::sync::RwLock::new(None))),
            browser_bridge: crate::routes::browser::BrowserBridge::new(),
            browser_registry: project_362_connect::connections::browser::BrowserRegistry::new(),
        ...
        // OwnedWebviewHandle.
        {
            use project_362_connect::connections::browser::UserBrowser;
        ...
        let server = Server::axum()
            ...
            // HTTP twin of the `project_362 export` CLI; used by the MCP export-video tool.
        ...
        let router = Router::new()
            ...
            // Local proxy → api.project-362.com/v1/chat/completions. Lets the
        ...
        let cm: crate::connections_api::SharedConnectionManager = Arc::new(Mutex::new(
            project_362_connect::connections::ConnectionManager::new(
                self.project_362_dir.clone(),
                self.secret_store.clone(),
            ),
        ));
        let wa: crate::connections_api::SharedWhatsAppGateway = Arc::new(Mutex::new(
            project_362_connect::whatsapp::WhatsAppGateway::new(self.project_362_dir.clone()),
        ));
        ...
        let mcp_store: crate::mcp_servers_api::SharedMcpServerStore =
            Arc::new(project_362_connect::mcp_servers::McpServerStore::new(
                self.project_362_dir.clone(),
                self.secret_store.clone(),
            ));
        let router = router.nest(
                ...
                axum::middleware::from_fn(
                    move |req: axum::extract::Request, next: axum::middleware::Next| {
                        ...
                        async move {
                            ...
                            // 2. project_362_auth=<token> cookie (img src, WebSocket)
                            ...
                            let cookie_token = req
                                ...
                                .and_then(|cookies| {
                                    cookies.split(';')
                                        .map(|c| c.trim())
                                        .find_map(|c| c.strip_prefix("project_362_auth="))
                                        .map(|s| s.to_string())
                                });
                            ...
                            if authorized {
                                ...
                                if upgrade {
                                    ...
                                    debug!(
                                        path = %path,
                                        "api auth: rejected WebSocket upgrade (missing/invalid token; use Cookie project_362_auth, Authorization Bearer, or ?token=)"
                                    );
                                }
                                axum::response::Response::builder()
                                    ...
                                    .body(axum::body::Body::from(
                                        ...
                                        r#"{"error":"unauthorized: API access requires authentication. Pass `Authorization: Bearer <your-api-key>`. Get the key with `project_362 auth token`, or set the `project_362_API_KEY` env var before starting project-362. (Desktop app users: Settings > Privacy.)"}"#,
                                    ))
                                    .unwrap()
                            }
                        }
