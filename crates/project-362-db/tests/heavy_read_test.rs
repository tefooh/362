...
//!   cargo test -p project-362-db --test heavy_read_test -- --nocapture
...
async fn open_real_db() -> Arc<DatabaseManager> {
    let db_path = project_362_core::paths::default_project_362_data_dir().join("db.sqlite");
...
/// still go through.
///
/// Requires the real production DB at `~/.project-362/db.sqlite` — ignored
/// in CI where that doesn't exist. Run locally with:
///   cargo test -p project-362-db --test heavy_read_test -- --ignored --nocapture
#[tokio::test]
#[ignore = "requires real ~/.project-362/db.sqlite"]
