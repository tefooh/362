...
mod tests {
    ...
    async fn test_frames_full_text_column_exists() {
        ...
        let result: Result<Vec<(String, String, String)>, _> = sqlx::query_as(
            "SELECT app_name, window_name, SUBSTR(full_text, 1, 200) as snippet FROM frames WHERE datetime(timestamp) > datetime('now', '-15 minutes') AND LENGTH(full_text) > 30 AND app_name != 'project_362' AND full_text IS NOT NULL ORDER BY timestamp DESC LIMIT 8"
        )
        .fetch_all(&db.pool)
        .await;
