...
/// Migrate legacy secret files from `~/.project-362` into the unified SecretStore.
///
...
pub async fn migrate_legacy_secrets(
    ...
    let fixed = fix_secret_file_permissions(project_362_dir);
    if fixed > 0 {
    ...
    let connections_path = project_362_dir.join("connections.json");
...
/// Walk `project_362_dir` and set 0o600 on any file matching secret-like patterns.
/// Returns the count of files whose permissions were fixed.
pub fn fix_secret_file_permissions(project_362_dir: &Path) -> usize {
    let mut count = 0;
    ...
    let whatsapp_dir = project_362_dir.join("whatsapp-session");
    let whatsapp_marker = project_362_dir.join(".whatsapp-perms-fixed");
