...
export interface RecorderOptions {
   ...
   * project_362 CLI writes, so an SDK-recorded session is queryable by
   * the existing `project-362-js` REST client or any tool that reads the
   * CLI's DB.
   *
   ...
   *   attached monitor**, matching the project_362 CLI which spins up
   ...
   * `project-362-a11y`'s `UiCaptureConfig::default()` ships with
 ...
 * 1-to-1 onto `project_362_a11y::config::UiCaptureConfig`. Each field
