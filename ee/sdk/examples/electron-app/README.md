# Electron example — `@project-362/sdk`
...
`@project-362/sdk/electron` helpers:
...
- **Native module loads cleanly in Electron's main process.** The `.node` prebuild from `bun run build` is referenced via `"@project-362/sdk": "file:../.."` in `package.json`.
- **First-class IPC helper**: `registerProject 362Ipc()` owns the `Recorder` instance and registers permission/start/stop/status/snapshot/reveal channels.
- **Preload bridge**: `exposeProject 362Api({ name: "api" })` exposes a context-isolated renderer API without enabling Node integration.
...
**`Error: Cannot find module '@project-362/sdk'` at launch**
