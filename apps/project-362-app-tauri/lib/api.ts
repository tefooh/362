...
/**
 * Single source of truth for local project_362 API access.
 *
 * All API calls to the project_362 backend MUST go through this module.
 * It handles:
...
function applyApiConfig(config: LocalApiConfig): void {
  ...
  if (_authEnabled && _apiKey && typeof document !== "undefined") {
    document.cookie = `project_362_auth=${_apiKey}; path=/; SameSite=Strict`;
  } else if (!_authEnabled && typeof document !== "undefined") {
    document.cookie = "project_362_auth=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
  }
}
...
/**
 * Get the base URL for the local project_362 API.
 */
...
/**
 * Fetch wrapper for the local project_362 API.
 *
