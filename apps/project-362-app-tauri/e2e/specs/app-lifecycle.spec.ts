...
describe("App lifecycle and UI stability", function () {
  ...
  it("round-trips localStorage", async () => {
    const ok = (await browser.execute(() => {
      localStorage.setItem("project-362-e2e-localstorage", "ok");
      const value = localStorage.getItem("project-362-e2e-localstorage");
      localStorage.removeItem("project-362-e2e-localstorage");
      return value === "ok";
    })) as boolean;
    expect(ok).toBe(true);
  });
  ...
  it("round-trips sessionStorage", async () => {
    const ok = (await browser.execute(() => {
      sessionStorage.setItem("project-362-e2e-sessionstorage", "ok");
      const value = sessionStorage.getItem("project-362-e2e-sessionstorage");
      sessionStorage.removeItem("project-362-e2e-sessionstorage");
      return value === "ok";
    })) as boolean;
    expect(ok).toBe(true);
  });
