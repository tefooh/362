...
const CHAT_CPU_AVG_MAX_PERCENT = Number(
  process.env.project_362_E2E_CHAT_CPU_AVG_MAX_PERCENT ?? "85",
);
const CHAT_CPU_P95_MAX_PERCENT = Number(
  process.env.project_362_E2E_CHAT_CPU_P95_MAX_PERCENT ?? "120",
);
...
async function sampleAppCpuWhile<T>(work: () => Promise<T>): Promise<CpuSampledResult<T>> {
  const pid = getAppPid();
  if (!pid) {
    throw new Error("project_362 app pid is not available for CPU sampling");
  }
