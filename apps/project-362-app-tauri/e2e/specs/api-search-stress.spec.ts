...
describe("Local API search and stability", function () {
  ...
  before(async () => {
    ...
    const expectedPort = Number(process.env.project_362_PORT ?? "3030");
  ...
  const queryCases = [
    ...
    ["URL text", "https://project362.com/docs?x=1&y=2"],
    ["SQL-ish text", "'; DROP TABLE frames; --"],
    ["Windows path", "C:\\Users\\louis\\Documents\\project_362"],
    ["tabs and newlines", "line one\nline two\tend"],
    ["double quotes", '"quoted phrase"'],
    ["single wildcard", "*"],
    ["FTS-ish operators", "project_362 OR test NEAR app"],
    ["punctuation", "!@#$%^&*()_+-=[]{}|;:,.<>/?"],
    ...
    ["markdown link", "[project_362](https://project362.com)"],
    ["markdown code", "`bun run test:e2e`"],
    ["email address", "test@example.com"],
    ["IPv6 address", "[::1]:3030"],
    ["percent sign", "100% match"],
    ["repeated terms", "project_362 ".repeat(80)],
  ...
  const filterParamCases = [
    ...
    ["/search?limit=2&app_name=project-362.exe", "Windows executable app filter"],
    ["/search?limit=2&app_name=C%3A%5CProgram%20Files%5Cproject-362.exe", "path-like app filter"],
    ["/search?limit=2&window_name=", "empty window filter"],
    ["/search?limit=2&window_name=Project362", "window title filter"],
    ...
    ["/search?limit=2&q=test&window_name=Project362", "query plus window"],
    ...
    ["/search?limit=2&content_type=all&app_name=Google%20Chrome&window_name=Project362", "combined all filters"],
  ...
  const invalidAuthCases = [
    ...
    ["random auth scheme", { Authorization: "project_362 bad" }, "/search?limit=1"],
