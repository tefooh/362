 ...
 *
 * Project362 runs a small local HTTP server (default port 11435) used for:
...
const FOCUS_SERVER_PORT = Number(process.env.project_362_FOCUS_PORT ?? "11436");
...
// HTTP (project_362_FOCUS_PORT handoff) and depends on OS-level window realization
...
describe.skip("Focus server", function () {
  ...
  it("forwards deep_link_url + args into the app via Tauri events", async function () {
    ...
    const deepLink = `project_362://e2e/deep-link?ts=${Date.now()}`;
