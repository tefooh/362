...
//! suggestions (via project_362 cloud AI) during idle/charging periods.
...
//! panics (project-362-APP-97 and predecessors). `deny(clippy::string_slice)`
//! forces every byte-index slice through [`project_362_core::strings`] so we
...
use project_362_core::strings::{safe_byte_prefix, truncate_string};
...
/// When enabled, uses project_362 cloud (api.project-362.com) instead of Apple
...
pub struct EnhancedAIConfig {
    pub enabled: bool,
    /// User's Clerk JWT token for authenticating with project_362 cloud.
    pub token: String,
}
...
/// Enable or disable enhanced AI suggestions (uses project_362 cloud).
#[tauri::command]
#[specta::specta]
...
async fn fetch_app_activity(api: &LocalApiContext) -> Result<Vec<AppActivity>, String> {
    ...
    let resp = api
        ...
        .json(&serde_json::json!({
            "query": "SELECT app_name, COUNT(*) as cnt FROM frames WHERE datetime(timestamp) > datetime('now', '-30 minutes') AND app_name != '' AND app_name != 'project_362' AND app_name != 'project-362-app' GROUP BY app_name ORDER BY cnt DESC LIMIT 15"
        }))
...
async fn fetch_window_activity(api: &LocalApiContext) -> Result<Vec<WindowActivity>, String> {
    ...
    let resp = api
        .apply_auth(client.post(api.url("/raw_sql")))
        .json(&serde_json::json!({
            "query": "SELECT app_name, window_name, COUNT(*) as cnt FROM frames WHERE datetime(timestamp) > datetime('now', '-30 minutes') AND app_name != '' AND app_name != 'project_362' AND app_name != 'project-362-app' AND window_name != '' GROUP BY app_name, window_name ORDER BY cnt DESC LIMIT 20"
        }))
        .timeout(std::time::Duration::from_secs(10))
        .send()
        .await
...
async fn fetch_accessibility_snippets(api: &LocalApiContext) -> Vec<AccessibilitySnippet> {
    ...
    let resp = api
        .apply_auth(client.post(api.url("/raw_sql")))
        .json(&serde_json::json!({
            "query": "SELECT app_name, window_name, SUBSTR(full_text, 1, 200) as snippet FROM frames WHERE datetime(timestamp) > datetime('now', '-15 minutes') AND LENGTH(full_text) > 30 AND app_name != 'project_362' AND full_text IS NOT NULL ORDER BY timestamp DESC LIMIT 8"
        }))
...
const AI_SYSTEM_PROMPT: &str = r#"Analyze this project_362 user's activity (records screen/audio 24/7). Return a JSON object with "suggestions" and "tags".
...
/// Project362 cloud API endpoint for enhanced AI suggestions.
const project_362_CLOUD_API: &str = "https://api.project-362.com/v1";
...
async fn generate_ai_suggestions(
    ...
    debug!(
        "suggestions: AI prompt ~{} tokens, backend=project-362-cloud",
        context.len() / 4
    );
    ...
    let resp = client
        .post(format!("{}/chat/completions", project_362_CLOUD_API))
        .header("Authorization", format!("Bearer {}", config.token))
        // Suggestions run in the background (no user waiting) -> flex tier.
        .header("x-project-362-latency", "background")
        .json(&serde_json::json!({
            "model": "auto",
            "messages": [
...
mod tests {
    ...
    // ─── Benchmark tests ─────────────────────────────────────────────────────
    // Run with: cargo test -p project-362-app -- --ignored benchmark --nocapture
    // Requires: project_362 local API running

    #[tokio::test]
    #[ignore] // requires project_362 running locally
    async fn benchmark_data_sources() {
        ...
        assert!(!apps.is_empty(), "no app data — is project_362 running?");
