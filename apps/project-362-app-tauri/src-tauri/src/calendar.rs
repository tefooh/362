...
pub async fn calendar_status() -> Result<CalendarStatus, String> {
    #[cfg(target_os = "macos")]
    {
        ...
        let auth_status = Project362Calendar::authorization_status();
...
async fn collect_calendar_events() -> Vec<CalendarEventItem> {
    #[cfg(target_os = "macos")]
    {
        ...
        if format!("{}", status) != "Full Access"
            && !Project362Calendar::access_granted_this_session()
        {
            return Vec::new();
        }
...
#[cfg(target_os = "macos")]
fn calendar_event_to_item(event: project_362_connect::calendar::CalendarEvent) -> CalendarEventItem {
...
fn calendar_event_to_item_win(
    event: project_362_connect::calendar_windows::CalendarEvent,
) -> CalendarEventItem {
    let start_display = event.start_local.format("%-I:%M %p").to_string();
    let end_display = event.end_local.format("%-I:%M %p").to_string();
