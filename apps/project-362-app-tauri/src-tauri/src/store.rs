...
use super::secrets;
use project_362_secrets::keychain;
...
/// recording start via `project_362_engine::auth_key::resolve_api_auth_key`,
...
/// project-362-app.YYYY-MM-DD.log.
...
fn decrypt_store_file(path: &Path) -> DecryptOutcome {
    ...
    match project_362_vault::crypto::decrypt_small(&data[8..], &key) {
        Ok(plaintext) => {
...
///
/// To opt in: create ~/.project-362/.encrypt-store or set project_362_ENCRYPT_STORE=1.
fn encrypt_store_file(path: &Path) {
    // Check opt-in flag
    let opted_in = std::env::var("project_362_ENCRYPT_STORE")
        .map(|v| v == "1")
        .unwrap_or(false)
        || path
            .parent()
    ...
    match project_362_vault::crypto::encrypt_small(&data, &key) {
        Ok(ciphertext) => {
...
pub struct SettingsStore {
    ...
    #[serde(flatten)]
    pub recording: project_362_config::RecordingSettings,
    ...
    pub user: User,
    #[serde(rename = "showProject362Shortcut")]
    pub show_project_362_shortcut: String,
    ...
    /// Use project_362 cloud for AI-powered features like suggestions.
    ...
    /// Disabled by default so the overlay doesn't appear in project_362's own recordings.
...
pub enum AIProviderType {
    ...
    #[serde(rename = "custom")]
    Custom,
    #[serde(rename = "project-362-cloud", alias = "claude-code")]
    Project362Cloud,
...
impl Default for AIPreset {
    fn default() -> Self {
        Self {
            id: String::new(),
            prompt: String::new(),
            provider: AIProviderType::Project362Cloud,
            url: "https://api.project-362.com/v1".to_string(),
...
impl AudioEngineFallbackReason {
    pub fn notification_title(&self) -> &'static str {
        match self {
            Self::NotLoggedIn | Self::NotSubscribed => "Project362 Cloud unavailable",
    ...
    pub fn notification_body(&self) -> &'static str {
        match self {
            Self::NotLoggedIn => {
                "You are not logged in, so audio is being transcribed locally with Whisper Turbo (fast). Log in to use Project362 Cloud."
            }
            Self::NotSubscribed => {
                "Project362 Cloud requires an active subscription, so audio is being transcribed locally with Whisper Turbo (fast)."
            }
...
impl Default for SettingsStore {
    fn default() -> Self {
        ...
        let default_free_preset = AIPreset {
            id: "project-362-cloud".to_string(),
            prompt: r#"IMPORTANT: At the start of every conversation, read the files in .pi/skills/ directory (e.g. .pi/skills/project-362-api/SKILL.md and .pi/skills/project-362-cli/SKILL.md) before responding.
Rules:
...
- Always wrap local file paths in angle brackets because project_362 paths often contain spaces or parentheses
...
"#.to_string(),
        ...
        Self {
            ...
            recording: project_362_config::RecordingSettings {
            ...
            user: User::default(),
            #[cfg(target_os = "windows")]
            show_project_362_shortcut: "Alt+S".to_string(),
            #[cfg(not(target_os = "windows"))]
            show_project_362_shortcut: "Super+Ctrl+S".to_string(),
            #[cfg(target_os = "windows")]
...
impl SettingsStore {
    ...
    fn sanitize_legacy_fields(mut val: Value) -> Value {
        if let Some(obj) = val.as_object_mut() {
            ...
            // Migrate unscoped "project_362" ignore-pattern to app-scoped "project_362::"
            // so browser tabs whose title contains "project_362" are no longer falsely
            ...
            if let Some(Value::Array(windows)) = obj.get_mut("ignoredWindows") {
                for entry in windows.iter_mut() {
                    if entry.as_str() == Some("project_362") {
                        *entry = Value::String("project_362::".to_string());
                    }
                }
            }
    ...
    pub fn to_recording_settings(&self) -> project_362_config::RecordingSettings {
        ...
        // This token is used as the Bearer credential for project_362 cloud
    ...
    pub fn to_recording_config(
    ...
    ) -> project_362_engine::RecordingConfig {
        let resolved_engine = self.audio_engine_resolution().active;
        let settings = self.to_recording_settings();
        let mut config = project_362_engine::RecordingConfig::from_settings(
            &settings,
            data_dir,
            Some(&resolved_engine),
        );
        // Resolve the API auth key from the seeded cache. The cache is populated
        // asynchronously by `recording::spawn_project_362` via the shared helper
        // (`project_362_engine::auth_key::resolve_api_auth_key`) — which is the
        ...
        // throwaway instance can run alongside another project_362 already on
        // :3030 instead of piggy-backing on it.
        if let Some(p) = std::env::var("project_362_PORT")
            .ok()
    ...
    fn audio_engine_resolution_with_cloud_auth(
        ...
        match engine.as_str() {
            "project-362-cloud" if !has_cloud_auth => {
                tracing::warn!("project-362-cloud selected but user not logged in, falling back to whisper-large-v3-turbo-quantized");
                resolution.active = fallback;
                resolution.fallback_reason = Some(AudioEngineFallbackReason::NotLoggedIn);
            }
            "project-362-cloud" if !is_subscribed => {
                tracing::warn!("project-362-cloud selected but user is not a pro subscriber, falling back to whisper-large-v3-turbo-quantized");
                resolution.active = fallback;
...
pub fn init_store(app: &AppHandle) -> Result<SettingsStore, String> {
    ...
    {
        let detected = project_362_config::detect_tier();
        ...
        if stored_tier != Some(detected) {
            ...
            if is_new_store || store.recording.device_tier.is_none() {
                project_362_config::apply_tier_defaults(&mut store.recording, detected);
            }
        ...
        if project_362_config::is_engine_unsafe(
            &store.recording.audio_transcription_engine,
            detected,
        ) {
            let safe = project_362_config::best_engine_for_platform(detected);
            tracing::warn!(
                "engine {} is unsafe on this platform (tier={:?}, macOS={:?}) — switching to {}",
                store.recording.audio_transcription_engine,
                detected,
                project_362_config::macos_major_version(),
                safe,
            );
    ...
    // recordings, and machine migrations often copy ~/.project-362 without the
...
mod tests {
    ...
    #[test]
    fn project_362_cloud_falls_back_when_not_logged_in() {
        let mut store = SettingsStore::default();
        ...
        assert_eq!(resolution.requested, "project-362-cloud");
    ...
    #[test]
    fn project_362_cloud_falls_back_when_not_subscribed() {
        let mut store = SettingsStore::default();
        store.recording.audio_transcription_engine = "project-362-cloud".to_string();
        store.user.token = Some("token".to_string());
        store.user.cloud_subscribed = Some(false);
    ...
    #[test]
    fn project_362_cloud_stays_active_for_app_entitled_users() {
        let mut store = SettingsStore::default();
        store.recording.audio_transcription_engine = "project-362-cloud".to_string();
        store.user.token = Some("token".to_string());
        store.user.cloud_subscribed = Some(false);
        ...
        assert_eq!(resolution.active, "project-362-cloud");
        assert_eq!(resolution.fallback_reason, None);
    }

    #[test]
    fn project_362_cloud_falls_back_for_stale_legacy_cloud_subscribed_without_entitlement() {
        let mut store = SettingsStore::default();
        store.recording.audio_transcription_engine = "project-362-cloud".to_string();
        store.user.token = Some("token".to_string());
        store.user.cloud_subscribed = Some(true);
    ...
    fn presets_n(n: usize) -> Vec<Value> {
        (0..n)
            .map(|i| json!({"id": format!("p{}", i), "model": "x", "provider": "project-362-cloud"}))
            .collect()
    }
