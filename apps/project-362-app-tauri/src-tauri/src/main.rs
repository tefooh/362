...
pub use recording::spawn_project_362;
pub use recording::stop_project_362;
...
use log_files::{get_log_files, get_project_362_data_dir};
use shortcuts::{
...
/// Returns which E2E seeds are requested (env project_362_E2E_SEED, comma-separated).
/// Rust uses "onboarding" in setup to complete onboarding at startup.
#[tauri::command]
#[specta::specta]
fn get_e2e_seed_flags() -> Vec<String> {
    std::env::var("project_362_E2E_SEED")
        .ok()
        .filter(|s| !s.is_empty())
        .map(|s| {
            s.split(',')
...
/// Returns true when project_362_SKIP_ONBOARDING is set to a truthy value
...
fn should_skip_onboarding() -> bool {
    std::env::var("project_362_SKIP_ONBOARDING")
        .ok()
...
async fn main() {
    ...
    // Refuse to launch while a `project_362 db recover|cleanup` operation is in
    // progress. The CLI writes ~/.project-362/.db_recovery.lock before doing
    ...
    //   1. `project_362 db unlock` — friendly path
    //   2. project_362_IGNORE_DB_LOCK=1 env var — bypass on this launch only
    //   3. `rm ~/.project-362/.db_recovery.lock` — manual
    //
    // See `crates/project-362-engine/src/cli/db.rs`.
    if std::env::var("project_362_IGNORE_DB_LOCK").ok().as_deref() != Some("1") {
        let lock_path =
            project_362_core::paths::default_project_362_data_dir().join(".db_recovery.lock");
        if let Ok(metadata) = std::fs::metadata(&lock_path) {
            ...
            if stale {
                ...
                eprintln!(
                    "project_362: a `project_362 db ...` operation is in progress.\n\
                     lock: {}\n\
                     content: {}\n\
                     options:\n  • wait for the op to finish, then re-open the app\n  • run `project_362 db unlock` if you're sure it's stuck\n  • set project_362_IGNORE_DB_LOCK=1 and retry to bypass this check",
                    lock_path.display(),
                    body.trim(),
                );
                std::process::exit(2);
    ...
    // zbus/tokio conflict) and acts as a fallback on macOS/Windows.
    {
        ...
        let deep_link_url = args
            .iter()
            .find(|a| a.starts_with("project_362://"))
            .cloned();

        let focus_port: u16 = std::env::var("project_362_FOCUS_PORT")
            .ok()
            ...
            .await
        {
            if resp.status().is_success() {
                eprintln!("project_362: another instance is already running — focused existing window, exiting.");
                std::process::exit(0);
            }
        }
    }
    ...
    let store_path = project_362_core::paths::default_project_362_data_dir().join("store.bin");
    let store_json = std::fs::read(&store_path).ok().and_then(|data| {
        if data.len() >= 8 && &data[..8] == b"SPSTORE1" {
            ...
            let plain = project_362_vault::crypto::decrypt_small(&data[8..], &key).ok()?;
    ...
    let telemetry_disabled = store_bool("analyticsEnabled")
        .map(|enabled| !enabled)
        .unwrap_or(false)
        || project_362_engine::analytics::telemetry_disabled_by_env();
    ...
    let sentry_guard = if !telemetry_disabled {
        Some(sentry::init((
            ...
            sentry::ClientOptions {
                release: Some(format!("project-362-app@{}", app_version).into()),
                send_default_pii: false,
                server_name: Some("project-362-app".into()),
                before_send: Some(std::sync::Arc::new(|mut event| {
                    ...
                    let build_time: u64 = env!("project_362_BUILD_UNIX_TIME")
                        .parse()
                        .unwrap_or(0);
                    if build_time > 0 {
    ...
    {
        let log_dir = project_362_core::paths::default_project_362_data_dir();
    ...
    let app = app.plugin(tauri_plugin_single_instance::init(|app, args, _cwd| {
        ...
        let _ = app.run_on_main_thread(move || {
            ...
            if let Some(url) = args_clone.iter().find(|a| a.starts_with("project_362://")) {
    ...
    let sync_scheduler = project_362_connect::sync_scheduler::SyncScheduler::new();
    ...
    let app = app.manage(recording_state)
        ...
        .setup(move |app| {
            ...
            #[cfg(target_os = "macos")]
            {
                ...
                let mut app_submenu_builder = SubmenuBuilder::new(app, "project_362")
                    .item(&PredefinedMenuItem::about(app, Some("About project_362"), None)?)
                    .separator();
                ...
                let app_submenu = app_submenu_builder
                    .item(
                        &MenuItemBuilder::with_id("quit_app", "Quit project_362")
            ...
            let log_dir = get_project_362_data_dir(app.handle())
                .unwrap_or_else(|_| project_362_core::paths::default_project_362_data_dir());
            let file_appender = RollingFileAppender::builder()
                .rotation(Rotation::DAILY)
                .filename_prefix("project-362-app")
                .filename_suffix("log")
                .max_log_files(5)
                .build(log_dir)?;
            ...
            // triggers the "project_362 is damaged" Gatekeeper popup and can
            ...
            // E2E seed: when project_362_E2E_SEED contains "no-recording", flip
            ...
            if e2e_flags.iter().any(|f| f == "cloud-audio-fallback") {
                ...
                store.recording.disable_vision = true;
                store.recording.audio_transcription_engine = "project-362-cloud".to_string();
                ...
                info!("E2E seed: project_362 cloud audio fallback");
            }
            ...
            // Resolve data directory from user setting (custom dir or ~/.project-362)
            let (data_dir, data_dir_fell_back) = config::resolve_data_dir(&store.data_dir);
            info!("Recording data directory: {}", data_dir.display());

            // Pin project_362_DATA_DIR to the *resolved* dir so every consumer of
            // `default_project_362_data_dir()` agrees with the engine on where the
            ...
            // `~/.project-362`. Tokens landed in one db.sqlite and were read from
            ...
            // can fire) makes `default_project_362_data_dir()` self-consistent and
            // also propagates the correct dir to child processes (the CLI
            // sidecar inherits this env).
            std::env::set_var("project_362_DATA_DIR", &data_dir);
            ...
            if !telemetry_disabled {
                sentry::configure_scope(|scope| {
                    ...
                    let telemetry_context = project_362_engine::telemetry_context::TelemetryContext::from_env();
            ...
            // E2E seed: when project_362_E2E_SEED contains "onboarding", mark onboarding complete
            let e2e_flags = get_e2e_seed_flags();
            ...
            // Escape hatch: project_362_SKIP_ONBOARDING=1 marks onboarding complete
            ...
            if should_skip_onboarding() && !onboarding_store.is_completed {
                match store::OnboardingStore::update(&app.handle(), |o| o.complete()) {
                    Ok(_) => {
                        info!("project_362_SKIP_ONBOARDING: onboarding marked complete");
                        onboarding_store.is_completed = true;
                    }
                    Err(e) => error!("project_362_SKIP_ONBOARDING: failed to complete onboarding: {}", e),
                }
            }
            ...
            //   reduces wait time after the final restart.
            {
                ...
                tauri::async_runtime::spawn(async move {
                    ...
                    let engine = match store_for_download.recording.audio_transcription_engine.as_str() {
                        "deepgram" | "project-362-cloud" => None, // Cloud engines don't need local model
                    ...
                    if let Some(engine) = engine {
                        ...
                        tokio::task::spawn_blocking(move || {
                            match project_362_audio::transcription::whisper::model::download_whisper_model(engine_clone) {
                    ...
                    let (_silero_result, _seg_result, _emb_result) = tokio::join!(
                        async {
                            match project_362_audio::vad::silero::SileroVad::ensure_model_downloaded().await {
                        ...
                        async {
                            ...
                            match project_362_audio::speaker::models::ensure_model_file(
                                project_362_audio::speaker::models::PyannoteModel::Segmentation
                            ).await {
                        ...
                        async {
                            match project_362_audio::speaker::models::ensure_model_file(
                                project_362_audio::speaker::models::PyannoteModel::Embedding
                            ).await {
            ...
            {
                let shortcut = store.show_project_362_shortcut.clone();
            ...
            //         && project_362_vault::crypto::is_encrypted_file(&data_dir.join("db.sqlite")).unwrap_or(false));
            // if vault_is_locked {
            ...
            'start_server: {
                ...
                if !store_clone.app_entitled_or_dev() {
                    info!("Skipping server auto-start: active project_362 plan required");
                ...
                // spawn_project_362 command. DB-wedge recovery consults this
                ...
                std::thread::Builder::new()
                    ...
                    .spawn(move || {
                        ...
                        server_runtime.block_on(async move {
                            ...
                            // request would 403. Mirrors the path in `spawn_project_362`.
                            ...
                            let owned_browser =
                                project_362_connect::connections::browser::OwnedBrowser::default_instance();
                            ...
                            // no recording required (project_362_E2E_SEED=...,search-fixture).
            ...
            // Setup tray
            if let Some(_) = app_handle.tray_by_id("project_362_main") {
            ...
            // degraded heuristic — detection now happens in `project-362-engine`
            ...
            // which only handles project-362-connect integrations.
            ...
            // telemetry builds with project_362_ENTERPRISE_LICENSE_KEY env set.
