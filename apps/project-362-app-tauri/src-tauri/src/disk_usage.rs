...
pub struct CachedDiskUsage {
    pub timestamp: i64,
    pub usage: DiskUsage,
    /// The project_362 data directory this entry was computed for. Used to
...
pub fn get_cache_dir() -> Result<Option<PathBuf>, String> {
    ...
    Ok(Some(proj_dirs.join("project_362")))
}
...
pub async fn disk_usage(
    ...
    info!(
        ...
        project_362_dir.display(),
        force_refresh
    );
    let data_dir = project_362_dir.join("data");
    ...
    let current_dir_key = canonical_dir_key(project_362_dir);
        ...
        if let Ok(content) = fs::read_to_string(&cache_file) {
            if content.contains("---") {
                ...
                let one_hour = 60 * 60; // 1 hour cache (reduced from 2 days)
                                        ...
                                        // `project_362_dir` defaults to "" on older cache entries — those
                                        ...
                                        // the same directory.
                let cached_key_normalized = canonical_dir_key(Path::new(&cached.project_362_dir));
                let dir_matches =
                    !cached.project_362_dir.is_empty() && cached_key_normalized == current_dir_key;
                ...
                if !dir_matches {
                    info!(
                        ...
                        cached.project_362_dir, current_dir_key
                    );
                }
            }
        }
    ...
    let total_data_size_bytes = directory_size(project_362_dir)
    ...
    for file_name in ["db.sqlite", "db.sqlite-wal", "db.sqlite-shm"] {
        let db_path = project_362_dir.join(file_name);
        if db_path.exists() {
    ...
    if let Ok(entries) = fs::read_dir(project_362_dir) {
    ...
    let available_space = {
        ...
        let path_obj = Path::new(&project_362_dir);
        let available = sys
            .disks()
            .iter()
    ...
    let cached = CachedDiskUsage {
        ...
        project_362_dir: current_dir_key,
    };
