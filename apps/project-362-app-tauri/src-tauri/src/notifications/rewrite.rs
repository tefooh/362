...
//! viewer deeplinks (`project_362://view?path=…`).
//!
...
/// the transformed string. Idempotent — already-rewritten `project_362://`
        ...
        .replace_all(body, |caps: &Captures| {
            ...
            match resolve_local_path(url) {
                Some(abs) => {
                    ...
                    format!("[{}](project_362://view?path={})", label, encoded)
                }
...
fn resolve_local_path(url: &str) -> Option<String> {
    ...
    if url.starts_with("project_362://") {
        return None;
    }
...
mod tests {
    ...
    fn rewrites_absolute_unix_path() {
        ...
        assert_eq!(
            out,
            "see [log](project_362://view?path=%2FUsers%2Fx%2Flog.md) here"
        );
    }
    ...
    fn rewrites_file_uri() {
        ...
        assert_eq!(out, "[x](project_362://view?path=%2Ftmp%2Fa.md)");
    }
    ...
    fn idempotent_on_viewer_links() {
        let body = "[x](project_362://view?path=%2Ftmp%2Fa.md)";
        assert_eq!(rewrite_file_links(body), body);
    }
    ...
    fn handles_multiple_links() {
        ...
        assert!(out.contains("project_362://view?path=%2Fx.md"));
        assert!(out.contains("https://y"));
        assert!(out.contains("project_362://view?path=%2Fz.json"));
    }

    #[test]
    fn rewrites_windows_drive_path() {
        let out = rewrite_file_links("[a](C:/foo/bar.md)");
        assert!(out.contains("project_362://view?path="));
    ...
    fn drops_fragment_on_file_uri() {
        ...
        assert!(out.contains("project_362://view?path=%2Ftmp%2Fa.md"));
    ...
    fn preserves_label_with_unicode() {
        ...
        assert!(out.starts_with("[📝 log](project_362://view?path="));
    }
}
