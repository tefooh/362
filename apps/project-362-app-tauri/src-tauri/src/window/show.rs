...
impl RewindWindowId {
    ...
    pub fn title(&self) -> &str {
        match self {
            RewindWindowId::Main => "Project 362",
            RewindWindowId::Home => "Project 362",
            RewindWindowId::Search => "Project 362",
            RewindWindowId::Onboarding => "Project 362",
...
impl ShowRewindWindow {
    ...
    fn show_existing_main(
        ...
        if overlay_mode == "window" {
            ...
            #[cfg(target_os = "windows")]
            {
                ...
                // Without this, re-showing an already-created overlay when another project_362
    ...
    pub fn show(&self, app: &AppHandle) -> tauri::Result<WebviewWindow> {
        ...
        let window = match self {
                                    ...
                                    std::thread::spawn(move || {
                                        ...
                                        #[cfg(target_os = "macos")]
                                        {
                                            ...
                                            let _ = app.run_on_main_thread(move || {
                                                ...
                                                // project_362 window (Settings, Chat), just clear.
                                        ...
                                        #[cfg(not(target_os = "macos"))]
                                        {
                                            ...
                                            if crate::commands::any_project_362_webview_has_focus(&app)
                                                && crate::commands::main_overlay_is_visible(&app)
                                            {
                                                info!(
                                                    "main-window blur: another project_362 window has focus, keep Escape registered"
                                                );
                            ...
                            if !is_focused {
                                ...
                                std::thread::spawn(move || {
                                    ...
                                    #[cfg(target_os = "macos")]
                                    {
                                        ...
                                        let _ = app.run_on_main_thread(move || {
                                            // Conditional restore: if focus moved to another
                                            // project_362 window, just clear. Only activate
                                    ...
                                    #[cfg(target_os = "windows")]
                                    {
                                        ...
                                        {
                                            info!(
                                                "Main overlay blur: another project_362 window has focus, keep Escape registered"
                                            );
            ...
            ShowRewindWindow::Home { page } => {
                ...
                // when Home blurs and no project_362 window has focus (user left the app).
                #[cfg(not(target_os = "macos"))]
                {
                        ...
                        if let tauri::WindowEvent::Focused(focused) = event {
                            if !focused {
                                ...
                                std::thread::spawn(move || {
                                    ...
                                    if crate::commands::any_project_362_webview_has_focus(&app_c) {
                                        return;
                                    }
                                    if crate::commands::is_main_close_in_progress() {
                                        return;
                                    ...
                                    std::thread::spawn(move || {
                                        info!(
                                            "shortcut-sync: unregister execute (reason=home_blur_no_project_362_focus)"
                                        );
                                        let _ = crate::commands::unregister_window_shortcuts_if_generation_unchanged(
                                            app2,
                                            expected_gen,
                                            "home_blur_no_project_362_focus",
                                        );
                                    });
                                });
                            } else {
    ...
    /// Used when transitioning from Main to another project_362 window (e.g. Home/Settings)
        ...
        if id.label() == RewindWindowId::Main.label() {
            #[cfg(target_os = "macos")]
            {
                ...
                run_on_main_thread_safe(app, move || {
                    ...
                    // we're transitioning to another project_362 window.
                });
            }
