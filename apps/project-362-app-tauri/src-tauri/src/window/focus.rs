...
/// Check if our own app (project_362) is still the active macOS application.
/// When the overlay panel loses focus to another project_362 window (Settings,
...
/// If our own app is still active (focus moved to another project_362 window
/// like Settings), we just clear without restoring — otherwise Settings/Chat
/// would lose keyboard focus. Also skip restoring if any project_362 window
...
pub(crate) fn restore_frontmost_app_if_external_with_app(app: Option<&AppHandle>) {
    ...
    if is_own_app_still_active() {
        info!("Focus moved to another project_362 window, clearing saved app without restoring");
        clear_frontmost_app();
        return;
    }
    ...
    if let Some(app) = app {
        ...
        for label in &non_panel_labels {
            if app.get_webview_window(label).is_some() {
                info!(
                    "Project362 '{}' window still open, clearing saved app without restoring",
                    label
                );
                clear_frontmost_app();
                return;
