...
pub fn init_magnify_handler(app: tauri::AppHandle) {
    ...
    if Class::get("Project362MagnifyHandler").is_none() {
        let superclass = Class::get("NSObject").unwrap();
        let mut decl = ClassDecl::new("Project362MagnifyHandler", superclass).unwrap();
    ...
    if Class::get("Project362ScrollInterceptor").is_none() {
        ...
        let superclass = Class::get("NSObject").unwrap();
        let decl = ClassDecl::new("Project362ScrollInterceptor", superclass).unwrap();
        decl.register();
    }
}
...
pub(crate) unsafe fn attach_magnify_gesture_to_view(view: tauri_nspanel::cocoa::base::id) {
    with_autorelease_pool(|| {
        ...
        let handler_class = match Class::get("Project362MagnifyHandler") {
            Some(cls) => cls,
            None => {
                tracing::warn!(
                    "Project362MagnifyHandler class not registered yet — skipping gesture attach"
                );
                return;
            }
        };

        // Check if we already added our recognizer (look for Project362MagnifyHandler target)
