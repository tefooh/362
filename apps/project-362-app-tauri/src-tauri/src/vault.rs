...
pub async fn vault_status(_app: AppHandle) -> Result<String, String> {
    ...
    if db_path.exists() {
        if let Ok(true) = project_362_vault::crypto::is_encrypted_file(&db_path) {
            return Ok("locked".to_string());
        }
    }
...
pub async fn vault_unlock(_app: AppHandle, password: String) -> Result<(), String> {
    use project_362_vault::crypto;

    let project_362_dir = project_362_core::paths::default_project_362_data_dir();

    // Read vault metadata and verify password
    let meta_path = project_362_dir.join("vault.meta");
    ...
    let _ = std::fs::remove_file(project_362_dir.join(".vault_locked"));
    ...
    tokio::spawn(async move {
        if data_dir.exists() {
            let (tx, _rx) =
                tokio::sync::watch::channel(project_362_vault::migration::MigrationProgress {
                    total_files: 0,
                    processed_files: 0,
                    total_bytes: 0,
                    processed_bytes: 0,
            ...
            if let Err(e) =
                project_362_vault::migration::decrypt_data_dir(&project_362_dir, &data_dir, key, tx)
                    .await
            {
