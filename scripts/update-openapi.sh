...
#
# Fetches the auto-generated OpenAPI spec from a running project-362 server,
...
curl -sf "http://${HOST}/openapi.yaml" > "$TMP" || {
  echo "ERROR: Could not reach project-362 at http://${HOST}"
...
content = content.replace(\"title: ''\", \"title: 'Project 362 API'\", 1)
