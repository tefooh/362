...
async function buildAndDeploy(ip: string): Promise<void> {
  const binary = `${REPO_ROOT}/target/release/project-362`;
  const debugBinary = `${REPO_ROOT}/target/debug/project-362`;
  ...
  if (await Bun.file(binary).exists()) {
    ...
    throw new Error(
      "No project_362 binary found. Run `cargo build -p project-362-engine --bin project_362` first."
    );
  }
  ...
  await scp(ip, src, "/tmp/project-362");
  await ssh(ip, "chmod +x /tmp/project-362");
}
...
async function startServer(ip: string, port = 3030): Promise<void> {
  await ssh(
    ip,
    `export PATH="/usr/local/bin:/tmp:$PATH" HOME=/Users/admin && nohup /tmp/project-362 record --disable-audio --port ${port} > /tmp/project-362.log 2>&1 & echo $!`,
    { timeout: 10_000 }
  );
...
async function stopServer(ip: string): Promise<void> {
  await ssh(ip, "pkill -f 'project_362 record' 2>/dev/null || true");
