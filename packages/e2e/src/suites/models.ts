...
export async function runModelTests(
  ...
  for (const t of engineTests) {
    ...
    try {
      ...
      await exec("pkill -f 'project_362 record' 2>/dev/null || true");
      ...
      const cmd = [
        ...
        `--port ${port}`,
        t.flags ?? "",
        `> /tmp/project-362-model-test.log 2>&1 & echo $!`,
      ].join(" ");
      ...
      const { stdout: logs } = await exec(
        "grep -i 'error\\|panic\\|fatal' /tmp/project-362-model-test.log | head -5",
        { timeout: 5_000 }
      );
