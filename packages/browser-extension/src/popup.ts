...
const SESSION_PAIR_KEY = "project_362_pending_pair";
...
async function tryFocusProject362(): Promise<void> {
  try {
    const ctrl = new AbortController();
    const t = setTimeout(() => ctrl.abort(), 2000);
    await fetch(project_362_FOCUS_URL, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
function setStatusUI(status: ConnStatus): void {
  ...
  const labels: Record<ConnStatus, string> = {
    ...
    server_down:   "project_362 not running",
  ...
  if (status === "ok") {
    ...
    hint.textContent = "Open the Project362 desktop app first.";
    section.hidden = false;
  }
}
...
async function runPairingFlow(baseUrl: string, existingPair?: PairState): Promise<void> {
  ...
  try {
    ...
    if (existingPair) {
      ...
      // Persist so polling can resume if popup closes when user switches to Project362.
      await saveSessionPair({ id: pairId, code, baseUrl, startedAt: Date.now() });
    }

    setPairingUI(code, "Switch to Project362 and click Allow.");
    bar.dataset.state = "bridge_down";
    text.textContent = "waiting for approval…";

    // Try to bring Project362 to front. The popup will close as soon as the
    // OS focuses another window — that's fine, session storage keeps state.
    await tryFocusProject362();
...
// ---------------------------------------------------------------------------
// Open Project362 flow (server_down state)
// ---------------------------------------------------------------------------

async function tryOpenProject362(baseUrl: string): Promise<void> {
  const btn = $<HTMLButtonElement>("open-btn");
  const hint = $<HTMLSpanElement>("action-hint");
  btn.disabled = true;
  hint.textContent = "Trying to open Project362…";
  ...
  if (status !== "server_down") {
    ...
    btn.disabled = false;
    hint.textContent = "Project362 is not running — please open it.";
  }
}
...
async function init(): Promise<void> {
  ...
  $<HTMLButtonElement>("open-btn").addEventListener("click", () => {
    void tryOpenProject362(baseUrl);
  });
