...
const project_362_FOCUS_URL = "http://127.0.0.1:11435/focus";
...
function setPairCode(code: string | null): void {
  ...
  details.open = false;
  el.textContent = code
    ? `match code: ${code}. approve only if the same code appears in project-362.`
    : "";
}
...
function setConnectedUi(connected: boolean): void {
  ...
  title.textContent = connected ? "project_362 connected" : "connect project_362";
  help.textContent = connected
    ? "you can close this tab. agents can now use your browser when needed."
    : "click connect, then approve the request in the project_362 desktop app. no API token copy/paste needed.";
}
...
async function openProject362ForApproval(): Promise<boolean> {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 2_000);
  try {
    const res = await fetch(project_362_FOCUS_URL, {
      method: "POST",
...
async function probeConnection(
  ...
  try {
    ...
    return {
      status: "server_down",
      message: `can't reach project_362 at ${baseUrl} — is the app running?`,
    };
  }
  ...
  try {
    ...
    if (auth.status === 401 || auth.status === 403) {
      return {
        ...
        message: token
          ? "token was rejected — copy a fresh one from project_362 Settings"
...
async function onConnectClick(): Promise<void> {
  ...
  setPairCode(null);
  const baseUrl = $<HTMLInputElement>("baseUrl").value.trim() || DEFAULT_BASE_URL;
  setStatus("saving", "checking for project_362…");

  const liveness = await probeConnection("", baseUrl);
  if (liveness.status === "server_down") {
    setStatus("server_down", `project_362 is not running at ${baseUrl}`);
  ...
  try {
    setStatus("pairing", "opening approval request in project_362…");
    const pair = await startPairing(baseUrl);
    setPairCode(pair.code);
    const focused = await openProject362ForApproval();
    setStatus(
      "pairing",
      focused
        ? "project_362 should come to front. click Allow there"
        : "approve in project-362. if it did not come forward, open the app manually"
    );
    ...
    if (approval.status === "approved") {
      ...
      setStatus(status, status === "ok" ? "connected to project_362" : message);
      setConnectedUi(status === "ok");
      return;
    }

    if (approval.status === "denied") {
      setStatus("denied", "connection denied in project_362");
      return;
    }
...
async function recheckSavedConnection(): Promise<void> {
  ...
  setStatus(
    status,
    status === "auth_required"
      ? "project_362 auth changed — connect again"
      : message
  );
}
...
async function init(): Promise<void> {
  ...
  if (token || baseUrl !== DEFAULT_BASE_URL) {
    ...
    setStatus("idle", "click connect, then approve in the project_362 app");
