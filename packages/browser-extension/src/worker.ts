...
/**
 * Service worker — maintains a single WebSocket to project_362's /browser/ws
...
async function connect(): Promise<void> {
  ...
  socket.onopen = () => {
    console.log("[project_362] connected");
  ...
  socket.onclose = () => {
    ...
    if (!openedThisAttempt) {
      ...
      if (closeWithoutOpen >= AUTH_FAIL_THRESHOLD) {
        setBadge("!", "#dc2626");
        notifyOnce(
          "Project362 extension needs a token",
          "Open the extension options to paste your project_362 API token."
        );
      }
    }
    scheduleReconnect();
...
function startHeartbeat(): void {
  ...
  heartbeatTimer = setInterval(() => {
    ...
    if (Date.now() - lastFrameAt > HEARTBEAT_DEAD_MS) {
      console.warn("[project_362] no server traffic for 50s — reconnecting");
      forceReconnect();
      return;
    }
    send({ type: "ping" });
...
chrome.alarms.create("project_362_keepalive", { periodInMinutes: 0.5 });
chrome.alarms.onAlarm.addListener((alarm) => {
  if (alarm.name !== "project_362_keepalive") return;
