 ...
 * (project-362-prod). The legacy SA-key path stays the default fallback.
 */
export interface WifConfig {
	...
	projectId: string; // GCP project the Vertex calls bill to (project-362-prod). When
 ...
 * Fixed project-362-prod values are defaulted in code so cutover only needs the
 * flag, the secret, and VERTEX_PROJECT_ID. See [[project_gateway_vertex_to_project_362_wif]].
 */
export function buildWifConfig(env: Env): WifConfig | undefined {
	...
	// project-362-prod is the WIF target (NOT VERTEX_PROJECT_ID, which stays = mediar
	...
	return {
		...
		issuer: env.WIF_JWT_ISSUER || 'https://api.project-362.com',
		jwtAudience: env.WIF_JWT_AUDIENCE || 'project-362-vertex-gateway',
		subject: env.WIF_JWT_SUBJECT || 'gateway',
		stsAudience:
			env.WIF_STS_AUDIENCE ||
			'//iam.googleapis.com/projects/7048263620/locations/global/workloadIdentityPools/project-362/providers/cf-worker',
...
export class VertexAIProvider implements AIProvider {
	 ...
	 *   3. impersonate the project_362 SA to get a real access token
	 * Verified end-to-end against project-362-prod 2026-06-22.
	 */
