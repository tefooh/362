...
describe('buildWifConfig — active config', () => {
	it('builds the project-362-prod config with baked-in defaults on a single flag flip', () => {
		...
		expect(w.issuer).toBe('https://api.project-362.com');
		expect(w.jwtAudience).toBe('project-362-vertex-gateway');
		expect(w.subject).toBe('gateway');
		expect(w.kid).toBe('sp-gateway-1');
		expect(w.stsAudience).toContain('workloadIdentityPools/project-362/providers/cf-worker');
