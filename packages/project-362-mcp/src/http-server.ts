...
/**
 * HTTP Server for Project362 MCP
 *
 ...
 *
 * Mirrors the project-362-engine CLI: --listen-on-lan flips bind to 0.0.0.0
...
export function parseArgs(argv: string[]): CliConfig {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    } else if (a === "--project-362-port" && argv[i + 1]) {
      project_362Port = parseInt(argv[++i], 10);
  ...
  if (Number.isNaN(project_362Port) || project_362Port <= 0 || project_362Port > 65535) {
    throw new CliError(`invalid --project-362-port: ${project_362Port}`);
  }
...
function usage(): string {
  return [
    "project-362-mcp http server",
    "",
    "  --port <n>             listen port (default 3031)",
    "  --project-362-port <n>  upstream project_362 API port (default 3030)",
...
const TOOLS = [
  {
    name: "search_content",
    description:
      "Search project_362's recorded content: screen text (OCR), audio transcriptions, and UI elements. " +
...
function makeFetchAPI(project_362Port: number) {
  // Honor project_362_API_URL so the HTTP MCP can also front a remote project_362
  // (set by `project_362 agent setup --api-url`); falls back to the local port.
  const base = (process.env.project_362_API_URL || `http://localhost:${project_362Port}`).replace(
    /\/+$/,
    "",
  );
...
function createMcpServer(fetchAPI: ReturnType<typeof makeFetchAPI>): Server {
  const s = new Server(
    { name: "project-362-http", version: "0.14.0" },
    { capabilities: { tools: {} } }
  );
...
export function buildHttpServer(config: CliConfig) {
  const fetchAPI = makeFetchAPI(config.project-362Port);
 ...
 * when invoked as `project-362-mcp --http …`, in addition to the direct
 * `project-362-mcp-http` bin path which auto-starts via the `isMain` check
 * below.
 */
export function runFromArgv(argv: string[]): void {
  ...
  server.listen(config.mcpPort, config.host, () => {
    ...
    console.log(`Project362 MCP HTTP server listening on ${printable}:${config.mcpPort}`);
